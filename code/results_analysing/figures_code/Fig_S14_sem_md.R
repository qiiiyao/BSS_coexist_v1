...
domin_sem1_md_all = psem(
  ...
  glmmTMB(mlgfd~#mpd_all+
            ...
            +(1|f_p)
          , family=gaussian, data=dat_dom_sps,
          control= glmmTMBControl(optCtrl = list(reltol = 1e-10, maxit = 1000),
...
sd.y_estab_sem1_md_all = sqrt(predict(estab_sem1_md_all[[1]],
                                        re.form=NA)%>%var+sum(unlist(VarCorr(estab_sem1_md_all[[1]])))+pi^2/3)
sd.y_estab_sem1_md_all
std.estab_sem1_md_all=unlist(fixef(estab_sem1_md_all[[1]]))[-1]*
  c(sd(dat_suc_sps$mnd),
    sd(dat_suc_sps$mlgfd),
    sd(dat_suc_sps$mpd_all),
    sd(dat_suc_sps$mconti_func_d_all))/sd.y_estab_sem1_md_all
...
std.domin_sem1_md_all=unlist(fixef(domin_sem1_md_all[[1]]))[-1]*
  c(sd(dat_dom_sps$mnd),
    sd(dat_dom_sps$mlgfd),
    sd(dat_dom_sps$mpd_all),
    sd(dat_dom_sps$mconti_func_d_all))/sd.y_domin_sem1_md_all
...
### Estab
coefs_estab_sem1_md_all = coefs_estab_sem1_md_all[-c(nrow(coefs_estab_sem1_md_all),
                                                     (nrow(coefs_estab_sem1_md_all)-1)),]
sem_estab_effects_md_all = data.frame(predictor = rep(c('mnd', 'mlgfd',
                                                          'mpd_all', 'mconti_func_d_all'), 3),
...
### Domin
coefs_domin_sem1_md_all = coefs_domin_sem1_md_all[-c(nrow(coefs_domin_sem1_md_all),
                                                     (nrow(coefs_domin_sem1_md_all)-1)),]
sem_domin_effects_md_all = data.frame(predictor = rep(c('mnd', 'mlgfd',
                                                          'mpd_all', 'mconti_func_d_all'), 3),
                                        type = rep(c('direct', 'indirect', 'total'),
                                                   each = 4),
                                        value = 0,
...
for (i in 1:length(unique(coefs_domin_sem1_md_all_c$Predictor))) {
  ...
  } else if(nrow(dat_3) != 1 & nrow(dat) > 2) {
    sem_domin_effects_md_all[sem_domin_effects_md_all$predictor == predictor_1&
...
sem_estab_effects_md_all.percent.plot = 
  ...
  scale_y_discrete(limits=c("mconti_func_d_all", "mpd_all", "mlgfd", "mnd"),
...
#ggthemr::ggthemr(palette = "fresh", layout = "clean")
sem_estab_effects_md_all.dir.mpd_all.plot = 
  ggplot((data=sem_estab_effects_md_all.longer%>% 
  ...
  theme(plot.title = element_text(family = 'Arial', face = 'plain', hjust = 0.5,
        ...
        panel.border = element_blank(),
        axis.line = element_line(color="#93785B", linewidth = 0.6),
        plot.margin = margin(t = 0.05, r = -0.05, b = -0.5, l = 0.45, unit = "cm"))
sem_estab_effects_md_all.dir.mpd_all.plot

sem_estab_effects_md_all.indir.mpd_all.plot = 
  ggplot((data=sem_estab_effects_md_all.longer %>% 
            filter(type == 'indirect')),aes(x=value,y=predictor,fill=predictor))+
  geom_col(width=0.8)+
  geom_vline(xintercept = 0, linetype = 3, color="grey40", linewidth = 0.8)+
  geom_vline(xintercept = -0.013, linetype = 2, color="#93785B", linewidth = 0.6)+
  scale_x_continuous(limits = c(-0.013, 0.013), breaks = seq(-0.01, 0.01, 0.01))+
  scale_y_discrete(limits=c("mconti_func_d_all", "mpd_all", "mlgfd", "mnd"),
                   position="right")+
  scale_fill_manual(values = c("#ccb974ff", "#dd8452ff", "#4c72b0ff", "#55a868ff"))+# color palatte in ggthemes
...
### Space for SEM
sem_estab_md_all_space1 = ggplot(NULL)+
...
# Direct and indirect effects
sem_domin_effects_md_all.dir.mpd_all.plot = 
  ggplot(data=(sem_domin_effects_md_all.longer %>% 
                 filter(type == 'direct')),aes(x=value,y=predictor,fill=predictor))+
  geom_col(width=0.8)+
  geom_vline(xintercept = 0, linetype = 3, color="grey40", linewidth = 0.8)+
  scale_x_continuous(limits = c(-0.16, 0.16), breaks = seq(-0.1, 0.1, 0.1))+
  scale_y_discrete(limits=c("mconti_func_d_all", "mpd_all", "mlgfd", "mnd"),
                   labels=#c(bquote('I-N MFD'[ab]),bquote('I-N MPD'[ab]),bquote('I-N MRFD'[ab]),bquote('I-N MND'[ab]))
                     c('MFD',
                       'MPD',
...
sem_domin_effects_md_all.indir.mpd_all.plot = 
  ggplot(data=(sem_domin_effects_md_all.longer %>% 
                 filter(type == 'indirect')),aes(x=value,y=predictor,fill=predictor))+
  geom_col(width=0.8)+
  geom_vline(xintercept = 0, linetype = 3, color="grey40", linewidth = 0.8)+
  geom_vline(xintercept = -0.04, linetype = 2, color="#93785B", linewidth = 0.6)+
  scale_x_continuous(limits = c(-0.04, 0.04), breaks = seq(-0.02, 0.02, 0.02))+
  scale_y_discrete(limits=c("mconti_func_d_all", "mpd_all", "mlgfd", "mnd"),
                   position="right")+
  scale_fill_manual(values = c("#ccb974ff", "#dd8452ff", "#4c72b0ff", "#55a868ff"))+# color palatte in ggthemes
  guides(fill="none")+
...
### Space for SEM
sem_domin_md_all_space1 = ggplot(NULL)+
  scale_y_continuous(limits = c(0,1))+
  scale_x_continuous(limits = c(0,1))+
  geom_hline(yintercept = 0.94, linetype = 1, color="#93785B", linewidth = 0.6)+
  geom_text(
...
sem_md_all_plot_all = ggdraw() +
  ...
  draw_plot(sem_estab_effects_md_all.dir.mpd_all.plot,
            x = 0, y = 0.5,
            width = 0.35, height = 0.18) +
  draw_plot(sem_estab_effects_md_all.indir.mpd_all.plot,
            x = 0.35, y = 0.5,
            width = 0.25, height = 0.18) +
  
  draw_plot(sem_estab_effects_md_all.percent.plot,
            x = 0.6, y = 0.5,
            width = 0.4, height = 0.18) +
  
  draw_plot(sem_estab_md_all_space1,
            x = 0.08, y = 0.68,
            width = 0.928, height = 0.32) +
  
  # ---- DOMINANCE BLOCK ---- #
  # Row 4: dir | indir | percent
  draw_plot(sem_domin_effects_md_all.dir.mpd_all.plot,
            x = 0, y = 0,
            width = 0.35, height = 0.18) +
  
  draw_plot(sem_domin_effects_md_all.indir.mpd_all.plot,
            x = 0.35, y = 0,
            width = 0.25, height = 0.18) +
  
  draw_plot(sem_domin_effects_md_all.percent.plot,
            x = 0.6, y = 0,
            width = 0.4, height = 0.18) +
  # Row 3: space1 plot full width
  draw_plot(sem_domin_md_all_space1,
            x = 0.08, y = 0.18,
            width = 0.928, height = 0.32) +
...
ggsave('results/figures/sem_md_all_plot_all.svg',
