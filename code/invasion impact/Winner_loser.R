...
sp_cover_f1 = sp_cover_f1 %>% filter(fake_age %in% seq(1,51,1))

trait = read.csv('data/original data/traits332.csv',
                 header = T)

### Step. 2: Calculation of yearly change of real time data 
species.change.fp.realtime = data.frame(FIELD=NULL, PLOT=NULL,
  ...
  for (j in 1:length(plot.list)){
    ...
    if (length(target.year.field)>1){
      ...
      for (k in 1:(length(n)-1)){
        ...
        species.change.fp.realtime = rbind(species.change.fp.realtime,
                                             data.frame(
          FIELD=field,
          PLOT=plot.list[j],
...
str(species.change.fp.realtime) #579449 obs. of  13 variables:
#species.change.fp.realtime2 = species.change.fp.realtime[!is.na(species.change.fp.realtime$absolute.change),]
# add some variables for analysis
species.change.fp.realtime$F_P = paste(species.change.fp.realtime$FIELD,
                                          species.change.fp.realtime$PLOT,
                                 sep = '_')
species.change.fp.realtime = species.change.fp.realtime %>% relocate(F_P, .after = PLOT)

save(species.change.fp.realtime,
     file = 'code/invasion impact/species.change.fp.realtime.rdata')


### Step. 3: Real time: Calculation of change at the species level at all time and moving windows

load('code/invasion impact/species.change.fp.realtime.rdata')

## All time
# making use of the data.table function to aggregate changes in cover 

species.change.fp.realtime2 = data.table(species.change.fp.realtime)
change.alltime.fp.realtime = species.change.fp.realtime2[,{n_pos = length(absolute.change[absolute.change>0]);
...
change.alltime.fp.realtime = change.alltime.fp.realtime %>% 
                             filter(species %in% trait[trait$Origin == 'Native',]$Species)
change.alltime.fp.realtime$gain_loss = NA
change.alltime.fp.realtime[change.alltime.fp.realtime$mean.relative.change < 0,]$gain_loss = 0
change.alltime.fp.realtime[change.alltime.fp.realtime$mean.relative.change > 0,]$gain_loss = 1
#save(change.alltime.fp.realtime, file = 'code/invasion impact/change_nati_alltime.rdata')
...
change.moving.fp.realtime.l = foreach(i = 1:14) %dopar% {
  ...
  species.change.fp.realtime2_t = species.change.fp.realtime2 %>% filter(
    to %in% seq_t
  ) 
  species.change.fp.realtime2_t = data.table(species.change.fp.realtime2_t)
  
  change.moving.fp.t = species.change.fp.realtime2_t[,{n_pos = length(absolute.change[absolute.change>0]);
  n_neg = length(absolute.change[absolute.change<0]);
  n_all = ifelse(n_pos+n_neg>0,n_pos+n_neg,1);
  est = binom.test(n_pos,n_all);
  list(n=length(absolute.change),
