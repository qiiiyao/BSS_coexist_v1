...
mod_cmd_4 = cmdstan_model(file_4)
...
for (i in plot_list[,1]) {
  ...
  sp_topf_5 = sp_order[1:5]
  
  sp = colnames(df)[posi_sp_start:ncol(df)]
  sp_topf_5_posi = which(sp %in% sp_topf_5)
  
  f_p = unique(df$f_p)
  df_l = lapply(fit_fp_top50_ages1_35[[i]][[3]][sp_topf_5_posi], function(x){
  ...
  fit_3 = mod_cmd_3$sample(data = stan.data,
                         seed = stan.seed, 
                         chains = chains, 
                         parallel_chains = chains,
                         iter_warmup = warmup,
                         ...
                         adapt_delta = 0.99)
  summary = fit$summary()
  summary_3 = fit_3$summary()
  summary_r_a = summary %>% filter(grepl("^gamma", variable) | grepl("^beta", variable))
  summary_3_r_a = summary_3 %>% filter(grepl("^gamma", variable) | grepl("^beta", variable))
  summary_r_a$mean-summary_3_r_a$mean
  head(fit$summary())
  head(fit_3$summary())
  #summary = fit$summary()
