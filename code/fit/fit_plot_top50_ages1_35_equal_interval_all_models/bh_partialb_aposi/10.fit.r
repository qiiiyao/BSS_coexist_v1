...
stan.seed = 52
file = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/noconstrained_competition_equal_interval/BH_partialb.stan')
...
fit_save = function(x) {
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi', '/posterior/',
...
fitted_plots_1 = list.files('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi/summary')
m = regexec("[0-9]+\\_[0-9]+", fitted_plots_1)
fitted_plots = unlist(regmatches(fitted_plots_1, m))
unfitted = setdiff(names(fit_fp_top50_ages1_35_equal_interval)[plot_list[,10]], fitted_plots)
...
for (i in unfitted_order) {
  ...
  sp = colnames(growD)[10:ncol(growD)]
   ...
   {cl = makeCluster(numer_cores-2)      
    ...
    #i = 2
    species = sp[i]
    fit = readRDS(paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi', '/posterior/',
                     unique(growD$f_p),'_', species, '.RDS'))
    summary = as.data.frame(fit$summary())}

    save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi',
                                  '/summary/','summary_',
    ...
    names(r_trans_sp) = sp
  save(a_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi', '/parameters/','a_',
                               unique(growD$f_p), '.rdata'))
  save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
  save(b_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi', '/parameters/','b_',
                               unique(growD$f_p), '.rdata'))                             
  stopCluster(cl)
}
  ...
  save(intra_trans, file =paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi', '/parameters/','intra_',
                                 unique(growD$f_p), '.rdata'))
  ...
  save(inter_all_trans, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_aposi', '/parameters/','inter_',
                                    unique(growD$f_p), '.rdata'))
