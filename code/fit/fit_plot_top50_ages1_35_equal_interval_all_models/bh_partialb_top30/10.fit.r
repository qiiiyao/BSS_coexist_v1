...
# Load data
load("/data/home/shpli3/my_pc/BSS/code/data preparation/transformed data/fit_fp_top30_ages1_35_equal_interval.RData")
...
fit_save = function(x) {
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30', '/posterior/',
...
#--------------------------------------------------
data_t = fit_fp_top30_ages1_35_equal_interval
plot_list = sapply(seq(1, 480, 48), function(x){y = c(x:(x+47))})
names(fit_fp_top30_ages1_35_equal_interval)[plot_list]

fitted_plots_1 = list.files('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30/summary')
m = regexec("[0-9]+\\_[0-9]+", fitted_plots_1)
fitted_plots = unlist(regmatches(fitted_plots_1, m))
unfitted = setdiff(names(fit_fp_top30_ages1_35_equal_interval)[plot_list[,10]], fitted_plots)
fitted_order = which(names(fit_fp_top30_ages1_35_equal_interval) %in% fitted_plots)
unfitted_order = which(names(fit_fp_top30_ages1_35_equal_interval) %in% unfitted)
...
for (i in unfitted_order) {
  ...
  sp = colnames(growD)[10:ncol(growD)]
   ...
   {cl = makeCluster(numer_cores-2)      
    ...
    #i = 2
    species = sp[i]
    fit = readRDS(paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30', '/posterior/',
                     unique(growD$f_p),'_', species, '.RDS'))
    summary = as.data.frame(fit$summary())}

    save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30',
                                  '/summary/','summary_',
    ...
    names(r_trans_sp) = sp
  save(a_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30', '/parameters/','a_',
                               unique(growD$f_p), '.rdata'))
  save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
  save(b_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30', '/parameters/','b_',
                               unique(growD$f_p), '.rdata'))                             
  stopCluster(cl)
}
  ...
  save(intra_trans, file =paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30', '/parameters/','intra_',
                                 unique(growD$f_p), '.rdata'))
  ...
  save(inter_all_trans, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh_partialb_top30', '/parameters/','inter_',
                                    unique(growD$f_p), '.rdata'))
