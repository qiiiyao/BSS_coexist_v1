...
stan.seed = 52
file = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/constrained_competition_equal_interval/Ricker_allposi.stan')
mod = cmdstan_model(file
#, stanc_options = list("allow-undefined")
)

file_uniform = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/constrained_competition_equal_interval/Ricker_allposi_uniform.stan')
mod_uniform = cmdstan_model(file_uniform
#, stanc_options = list("allow-undefined")
)
...
fit_save = function(x) {
  #x = 2
  species = sp[x]
  fit = mod_uniform$sample(data = datalist[[x]],
                   seed = stan.seed, 
                   chains = chains, 
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker', '/posterior/',
...
names(data_t)[plot_list[,1]]

fitted_plots_1 = list.files('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker/summary')
m = regexec("[0-9]+\\_[0-9]+", fitted_plots_1)
fitted_plots = unlist(regmatches(fitted_plots_1, m))
unfitted = setdiff(names(data_t)[plot_list[,1]], fitted_plots)
...
for (i in unfitted_order) {
  ...
  sp = colnames(growD)[10:ncol(growD)]
   ...
   {cl = makeCluster(numer_cores-2)      
    ...
    #i = 2
    species = sp[i]
    fit = readRDS(paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker', '/posterior/',
                     unique(growD$f_p),'_', species, '.RDS'))
    summary = as.data.frame(fit$summary())}

    save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker',
                                  '/summary/','summary_',
    ...
    names(r_trans_sp) = sp
  save(a_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker', '/parameters/','a_',
                               unique(growD$f_p), '.rdata'))
  save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
  stopCluster(cl)
}
  ...
  for (i in 1:length(sp)) {
    ...
    for (j in i+1:(length(sp))){
      ...
      fitness.diff.ij = ((ri/rj) * sqrt( (ajj*aji)/(aii*aij) ))
      
      fitness.diff.ji = ((rj/ri) * sqrt( (aii*aij)/(ajj*aji) ))
  ...
  save(intra_trans, file =paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker', '/parameters/','intra_',
                                 unique(growD$f_p), '.rdata'))
  ...
  save(inter_all_trans, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker', '/parameters/','inter_',
                                    unique(growD$f_p), '.rdata'))
