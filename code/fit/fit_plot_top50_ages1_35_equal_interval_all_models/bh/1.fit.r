...
file = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/constrained_competition_equal_interval/BH_allposi.stan')
mod = cmdstan_model(file
#, stanc_options = list("allow-undefined")
)

file_uniform = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/constrained_competition_equal_interval/BH_allposi_uniform.stan')
...
fit_save = function(x) {
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh', '/posterior/',
...
names(fit_fp_top50_ages1_35_equal_interval)[plot_list[,1]]
...
for (i in plot_list[,1]) {
  ...
  sp = colnames(growD)[10:ncol(growD)]
   ...
   {cl = makeCluster(numer_cores-2)      
    ...
    #i = 2
    species = sp[i]
    fit = readRDS(paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh', '/posterior/',
                     unique(growD$f_p),'_', species, '.RDS'))
    summary = as.data.frame(fit$summary())}

    save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh',
                                  '/summary/','summary_',
    ...
    rownames(a_trans_sp) = sp
    names(r_trans_sp) = sp
  save(a_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh', '/parameters/','a_',
                               unique(growD$f_p), '.rdata'))
  save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
  stopCluster(cl)
}
  ...
  for (i in 1:length(sp)) {
    ...
    for (j in i+1:(length(sp))){
      ...
      fitness.diff.ij = ((exp(ri)-1)/(exp(rj)-1)) * sqrt( (ajj*aji)/(aii*aij) )
      
      fitness.diff.ji = ((exp(rj)-1)/(exp(ri)-1)) * sqrt( (aii*aij)/(ajj*aji) )
  ...
  save(intra_trans, file =paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh', '/parameters/','intra_',
                                 unique(growD$f_p), '.rdata'))
  ...
  save(inter_all_trans, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/bh', '/parameters/','inter_',
                                    unique(growD$f_p), '.rdata'))
