...
stan.seed = 52
file = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/constrained_competition_equal_interval/Ricker_log(n+1)_allposi.stan')
...
fit_save = function(x) {
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker_logn1', '/posterior/',
...
names(data_t)[plot_list[,10]]

fitted_plots_1 = list.files('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker_logn1/summary')
m = regexec("[0-9]+\\_[0-9]+", fitted_plots_1)
fitted_plots = unlist(regmatches(fitted_plots_1, m))
unfitted = setdiff(names(data_t)[plot_list[,10]], fitted_plots)
...
for (i in unfitted_order) {
  ...
  sp = colnames(growD)[10:ncol(growD)]
   ...
   {cl = makeCluster(numer_cores-2)      
    ...
    #i = 2
    species = sp[i]
    fit = readRDS(paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker_logn1', '/posterior/',
                     unique(growD$f_p),'_', species, '.RDS'))
    summary = as.data.frame(fit$summary())}

    save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker_logn1',
                                  '/summary/','summary_',
    ...
    names(r_trans_sp) = sp
  save(a_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker_logn1', '/parameters/','a_',
                               unique(growD$f_p), '.rdata'))
  save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker_logn1', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
  stopCluster(cl)
}
  ...
  save(intra_trans, file =paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker_logn1', '/parameters/','intra_',
                                 unique(growD$f_p), '.rdata'))
  ...
  save(inter_all_trans, file=paste0('fit_results/plot_ages1_35_top50_equal_interval_model_comparison/ricker_logn1', '/parameters/','inter_',
                                    unique(growD$f_p), '.rdata'))
