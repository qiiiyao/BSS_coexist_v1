...
stan.seed = 52
file = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/constrained_competition/Ricker_log(n+1)_allposi.stan')
...
fit_save = function(x) {
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_model_comparison/ricker_logn1', '/posterior/',
...
names(fit_fp_top50_ages1_35)[plot_list[,10]]

for (i in plot_list[,10]) {
  #i = 1
  datalist = data_t[[i]][[3]]
  growD = data_t[[i]][[2]]
  unique(growD$f_p)
  sp = colnames(growD)[10:ncol(growD)]
   ...
   {cl = makeCluster(numer_cores-2)      
    ...
    #i = 2
    species = sp[i]
    fit = readRDS(paste0('fit_results/plot_ages1_35_top50_model_comparison/ricker_logn1', '/posterior/',
                     unique(growD$f_p),'_', species, '.RDS'))
    summary = as.data.frame(fit$summary())}

    save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/ricker_logn1',
                                  '/summary/','summary_',
    ...
    names(r_trans_sp) = sp
  save(a_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/ricker_logn1', '/parameters/','a_',
                               unique(growD$f_p), '.rdata'))
  save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/ricker_logn1', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
  stopCluster(cl)
}
  ...
  for (i in 1:length(sp)) {
    ...
    for (j in i+1:(length(sp))){
      ...
      fitness.diff.ij = ((ri/rj)*sqrt( abs(((ajj)*(aji))/((aii)*(aij))) ))
      
      fitness.diff.ji = ((rj/ri)*sqrt( abs(((aii)*(aij))/((ajj)*(aji))) ))
  ...
  save(intra_trans, file =paste0('fit_results/plot_ages1_35_top50_model_comparison/ricker_logn1', '/parameters/','intra_',
                                 unique(growD$f_p), '.rdata'))
  ...
  save(inter_all_trans, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/ricker_logn1', '/parameters/','inter_',
                                    unique(growD$f_p), '.rdata'))
