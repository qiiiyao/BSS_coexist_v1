...
stan.seed = 52
file = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/constrained_competition/Exp_r_allposi.stan')
...
fit_save = function(x) {
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_model_comparison/null', '/posterior/',
...
for (i in plot_list[,10]) {
  ...
  {cl = makeCluster(numer_cores-2)      
    ...
    foreach(x=1:length(datalist), .packages = c('dplyr', 'cmdstanr')) %dopar% {fit_save(x)
      ...
      summary_l = foreach(i=1:length(datalist)) %dopar% {
        #i = 2
      species = sp[i]
      fit = readRDS(paste0('fit_results/plot_ages1_35_top50_model_comparison/null', '/posterior/',
                     unique(growD$f_p),'_', species, '.RDS'))
      summary = as.data.frame(fit$summary())}

      save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/null',
                                  '/summary/','summary_',
  ...
  # Extract results
   ...
   save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/null', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
   stopCluster(cl)
   }
}