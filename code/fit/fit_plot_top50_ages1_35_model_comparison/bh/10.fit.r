...
stan.seed = 52
file = file.path('/data/home/shpli3/my_pc/BSS/code/fit/stancode/model_comprison/constrained_competition/BH_allposi.stan')
...
fit_save = function(x) {
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_model_comparison/bh', '/posterior/',
...
for (i in plot_list[,10]) {
  ...
  sp = colnames(growD)[10:ncol(growD)]
   ...
   {cl = makeCluster(numer_cores-2)      
    ...
    #i = 2
    species = sp[i]
    fit = readRDS(paste0('fit_results/plot_ages1_35_top50_model_comparison/bh', '/posterior/',
                     unique(growD$f_p),'_', species, '.RDS'))
    summary = as.data.frame(fit$summary())}

    save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/bh',
                                  '/summary/','summary_',
    ...
    names(r_trans_sp) = sp
  save(a_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/bh', '/parameters/','a_',
                               unique(growD$f_p), '.rdata'))
  save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/bh', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
  stopCluster(cl)
}
  ...
  save(intra_trans, file =paste0('fit_results/plot_ages1_35_top50_model_comparison/bh', '/parameters/','intra_',
                                 unique(growD$f_p), '.rdata'))
  ...
  save(inter_all_trans, file=paste0('fit_results/plot_ages1_35_top50_model_comparison/bh', '/parameters/','inter_',
                                    unique(growD$f_p), '.rdata'))
