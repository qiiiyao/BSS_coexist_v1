...
transformed parameters {
 ...
 // Rciker model 
  vector[N] yhat;
  for (i in 1:N) {
    yhat[i] = (r+log_X1[i]*a+ran_t[time[i]]+ran_f[field[i]])*time_diff[i];
  }
}
...
model {
  ...
  // likelihood
  log_G ~ normal(yhat, sigma_e); // robust regression using student distribution following McElreath_2020
}
