...
for (z in  seq(1, 480, by = 24)) {
  ...
  growD_domin = re_cover_ab_domin_l_f[[z]]
...
#  if(sdscal != 0) {
    ...
    for (j in 1:length(sp)) {
      ...
      summary = summary_l[[j]]
      write.table(summary, paste('results/summary_single/',unique(growD$f_p), '_', sp[j], '.txt', sep = ''))
    }
    ...
    write.table(a_trans_sp_inver, file=paste('results/parameters_single/a_',
                                             unique(growD$f_p),
                                             ".txt",sep = ''))
    write.table(r_trans_sp, file=paste('results/parameters_single/r_',
                                       unique(growD$f_p),
                                       ".txt",sep = ''))
    ...
    write.table(intra_trans, file=paste('results/parameters_single/intra_',unique(growD$f_p), ".txt",sep = ''))
    ...
    write.table(inter_all_trans, file=paste('results/parameters_single/inter_',unique(growD$f_p), ".txt",sep = ''))
    #write.csv(inter_all_trans, file=paste('resultsparameters_single/inter_',unique(growD$f_p), ".csv",sep = ''))
