...
stan.seed = 52
file = file.path('D:/R projects/BSS/code/fit/stancode/model_comprison/Ricker_random_time_prop_cover_loo_aposi.stan')
...
fit_save = function(x) {
  ...
  # Raw output
  #------------ 
  fit$save_object(file = paste0('fit_results/plot_ages1_35_top50_propcover_aposi', '/posterior/',
...
#--------------------------------------------------
data_t = fit_fp_top50_ages1_35_cover_prop
plot_list = sapply(seq(1, 480, 48), function(x){y = c(x:(x+47))})

for (i in plot_list[,1]) {
  #i = 1
  datalist = data_t[[i]]$fit_data_prop
  growD = data_t[[i]]$re_cover_ab
  unique(growD$f_p)
  ...
  summary_l = foreach(i=1:length(datalist)) %dopar% {
    #i = 2
    species = sp[i]
    fit = readRDS(paste0('fit_results/plot_ages1_35_top50_propcover_aposi', '/posterior/',
                         unique(growD$f_p),'_', species, '.RDS'))
    summary = as.data.frame(fit$summary())}
  
  save(summary_l, file=paste0('fit_results/plot_ages1_35_top50_propcover_aposi',
                              '/summary/','summary_',
  ...
  a_trans_sp = a_trans_sp ### consistent with Fi = ri-aijNj
  colnames(a_trans_sp) = sp
  rownames(a_trans_sp) = sp
  save(a_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_propcover_aposi', '/parameters/','a_',
                               unique(growD$f_p), '.rdata'))
  save(r_trans_sp, file=paste0('fit_results/plot_ages1_35_top50_propcover_aposi', '/parameters/','r_',
                               unique(growD$f_p), '.rdata'))
  stopCluster(cl)
  ...
  for (i in 1:length(sp)) {
    ...
    for (j in i+1:(length(sp))){
      ...
      fitness.diff.ij = 1-((ri/rj)*sqrt( abs(((ajj)*(aji))/((aii)*(aij))) ))
      
      fitness.diff.ji = 1-((rj/ri)*sqrt( abs(((aii)*(aij))/((ajj)*(aji))) ))
  ...
  save(intra_trans, file =paste0('fit_results/plot_ages1_35_top50_propcover_aposi', '/parameters/','intra_',
                                 unique(growD$f_p), '.rdata'))
  ...
  save(inter_all_trans, file=paste0('fit_results/plot_ages1_35_top50_propcover_aposi', '/parameters/','inter_',
                                    unique(growD$f_p), '.rdata'))
