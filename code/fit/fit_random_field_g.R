...
load("code/Yin_ele_2022/sp_cover_f2_top50.rdata")

names(fit_yin_ele) = c('BSSzmat.dat', 'BSScover.dat', 'BSSothercover.dat', 'BSSotherrich.dat',
                       'growth.list')
BSScover.dat = array(unlist(fit_yin_ele$BSScover.dat), dim = c(480, 42, 50))
BSScover.dat = aperm(BSScover.dat, c(2, 3, 1))
growth.list = array(unlist(fit_yin_ele$growth.list), dim = c(480, 41, 50))
growth.list = aperm(growth.list, c(2, 3, 1))

dom = sp_cover_f2_top50
...
plot_list = sapply(seq(1, 50, 5), function(x){y = c(x:(x+4))})
