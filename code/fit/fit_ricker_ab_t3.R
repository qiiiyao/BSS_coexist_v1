...
for (z in seq(1, 480, by = 24)) {
  #z = 67
  growD_Estab = re_cover_ab_estab_t3_l_f[[z]]
  growD_Intro = re_cover_ab_intro_t3_l_f[[z]]
  growD_Nati = re_cover_ab_nati_t3_l_f[[z]]
  growD_domin = re_cover_ab_domin_t3_l_f[[z]]
  if (is.null(growD_Nati)) {print('No t3 for this plot!')} else {
  ...
  for (j in 1:length(sp)) {
    ...
    summary = summary_l[[j]]
    write.table(summary, paste('D:/BSS study/results/time_sep/summary/',unique(growD$f_p), '_', sp[j], '_t3.txt', sep = ''))
  }
  ...
  write.table(intra_trans, file=paste('D:/BSS study/results/time_sep/parameters/intra_',unique(growD$f_p), "_t3.txt",sep = ''))
  ...
  write.table(inter_all_trans, file=paste('D:/BSS study/results/time_sep/parameters/inter_',unique(growD$f_p), "_t3.txt",sep = ''))
