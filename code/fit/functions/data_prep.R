...
data_prep <- function(perform, # column name for performance indicator
                      focal,   # column name for species or element indicator
                      nonNcols,      # number of columns before neighbour abundances begin
                      #perform = 'G', 
                      #focal = 'focal', 
                      #nonNcols = 2,
  ...
  stan.data$ftime <- length(unique(df$time))   # length of year
  stan.data$time <- df$time           # yearly effects

  
  # VECTORS
  stan.data$species_ID <- as.numeric(as.factor(df[ , focal])) 
  stan.data$perform <- df[,perform]
  stan.data$time_diff = df$time_diff
