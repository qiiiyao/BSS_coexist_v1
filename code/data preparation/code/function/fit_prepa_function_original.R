fit_prepa_original = function(w, t = NULL, seq_t_all, nati_per, intro_per, estab_per,
                              domin_per) {
  #w = sp_cover_f3_fp[[36]]
  #t = 2:14
  #t = NULL
  ...
  if (is.null(t)) { 
    re_cover_ab = cbind(w[,1:7],
                        w[,8:(ncol(w))][colSums(w[,
                                                  8:(ncol(w))]) != 0])
  } else { 
    t_seq = seq_t_all[t]
    w_t = w[w$Age %in% t_seq,]
    re_cover_ab = cbind(w_t[,1:7],
                        w_t[,8:(ncol(w_t))][colSums(w_t[,
                                                        8:(ncol(w_t))]) != 0])
  }
  re_cover_ab = as.data.frame(re_cover_ab) ## ungrouped grouped dataframe 
  re_cover_ab_inva = cbind(re_cover_ab[,1:7],
                           re_cover_ab %>%
                             select(all_of(intersect(colnames(re_cover_ab)[8:ncol(re_cover_ab)],
  ...
  re_cover_prop = cbind(re_cover_ab[,1:7],
                        t(apply(re_cover_ab[,8:(ncol(re_cover_ab))], 1, function(x){x/sum(x)})))
  re_cover_prop_inva = cbind(re_cover_prop[,1:7],
                             re_cover_prop %>% 
                               select(all_of(intersect(colnames(re_cover_prop)[8:ncol(re_cover_prop)],
                                                       unlist(trait %>% filter(Origin == 'Exotic') %>% select(Species))))))
  ...
  for (j in 1:(ncol(re_cover_prop_inva)-7)) {
    #j = 15
    
    focal_alien = re_cover_prop_inva[,c(2, 7+j)]
  ...
  re_cover_prop_intro = cbind(re_cover_prop[,1:7],
                              re_cover_prop %>%
                                select(all_of(setdiff(colnames(re_cover_prop_inva)[8:ncol(re_cover_prop_inva)], estab_sp))))
  re_cover_ab_intro = cbind(re_cover_ab[,1:7],
                            re_cover_ab %>%
                              select(all_of(setdiff(colnames(re_cover_ab_inva)[8:ncol(re_cover_ab_inva)], estab_sp))))
  ...
  sp_estab_f = NULL
    ...
    sr_estab = ncol(f)-7
    sp_estab_f = colnames(re_cover_ab_estab_f)[8:ncol(re_cover_ab_estab_f)]}
  ...
  if(is.null(z_1)){
    ...
    sp_intro_f = NULL} else {
      ...
      sr_intro = ncol(f_1)-7
      sp_intro_f = colnames(re_cover_ab_intro_f)[8:ncol(re_cover_ab_intro_f)]}
  ...
  sp_nati_f = NULL
    ...
    sr_nati = ncol(f_2)-7
    sp_nati_f = colnames(re_cover_ab_nati_f)[8:ncol(re_cover_ab_nati_f)]}
  ...
  sp_domin_f = NULL
    ...
    sr_domin = ncol(f_3)-7
    sp_domin_f = colnames(re_cover_ab_domin_f)[8:ncol(re_cover_ab_domin_f)]}
  ...
  if (is.null(re_cover_ab_intro_f)) {
    ...
    col_intro = ncol(as.data.frame(re_cover_ab_intro_f[,8:ncol(re_cover_ab_intro_f)]))
  }
  if (is.null(re_cover_ab_estab_f)) {
    col_estab = 0
  } else {
    col_estab = ncol(as.data.frame(re_cover_ab_estab_f[,8:ncol(re_cover_ab_estab_f)]))
  }
  
  if (col_intro>0 & col_estab>0) {
    re_cover_ab_f = cbind(re_cover_ab_nati_f, 
                          re_cover_ab_intro_f[,8:ncol(re_cover_ab_intro_f)],
                          re_cover_ab_estab_f[,8:ncol(re_cover_ab_estab_f)])
  } else if (col_intro>0) {
    re_cover_ab_f = cbind(re_cover_ab_nati_f, 
                          re_cover_ab_intro_f[,8:ncol(re_cover_ab_intro_f)])
  } else if (col_estab>0) {
    re_cover_ab_f = cbind(re_cover_ab_nati_f, 
                          as.data.frame(re_cover_ab_estab_f[,8:ncol(re_cover_ab_estab_f)]))
  } else {
    re_cover_ab_f = cbind(re_cover_ab_nati_f)
  }
  
  ### Create the data list for stan fit 
  colnames(re_cover_ab_f)[8:ncol(re_cover_ab_f)] = c(sp_nati_f, sp_intro_f, sp_estab_f)
  sp = colnames(re_cover_ab_f)[8:ncol(re_cover_ab_f)]
  ...
  sp_matrix = re_cover_ab_f[1:(nrow(re_cover_ab_f)-1),8:ncol(re_cover_ab_f)]
