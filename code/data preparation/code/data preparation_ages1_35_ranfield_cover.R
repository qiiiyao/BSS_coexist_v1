...
source('code/data preparation/code/function/fit_prepa_function_top50_prop_cover.R')
...
fit_fp_top50_ages1_35_cover_prop = lapply(sp_cover_f1_ages1_35_top50_fp,
                                          fit_top50_prop_cover)
intro_sp_top50_early_suc = sapply(fit_fp_top50_ages1_35_cover_prop,
...
domin_sp_top50_early_suc = sapply(fit_fp_top50_ages1_35_cover_prop,
                                  function(x){length(strsplit(x[["stage_sp_name"]][4], ', ')[[1]])})
length(intro_sp_top50_early_suc[which(intro_sp_top50_early_suc == 0)])/length(intro_sp_top50_early_suc)
mean(sapply(fit_fp_top50_ages1_35_cover_prop,
            function(x){ncol(x[["re_cover_ab"]])-9})) ## mean 37 species
abundance_pro = mean(sapply(fit_fp_top50_ages1_35_cover_prop,
                            function(x){sum(x[["re_cover_ab"]][,c(10:ncol(x[["re_cover_ab"]]))])})/abundance_raw)

richness_pro = mean(sapply(fit_fp_top50_ages1_35_cover_prop,
                           function(x){ncol(x[["re_cover_ab"]][,c(10:ncol(x[["re_cover_ab"]]))])})/richness_raw)

save(fit_fp_top50_ages1_35_cover_prop,
     file = "code/data preparation/transformed data/fit_fp_top50_ages1_35_cover_prop.RData")

#### top50 prepare for random_field fitting #####
library(tidyr)
load("code/data preparation/transformed data/fit_fp_top50_ages1_35_cover_prop.RData")
freq_top50_sps = colnames(sp_cover_f1_ages1_35_top50)[10:ncol(sp_cover_f1_ages1_35_top50)]
fit_ranfield_top50_ages1_35_cover_prop = lapply(fit_fp_top50_ages1_35_cover_prop, function(x){
 # x = fit_fp_top50_ages1_35_cover_prop[[1]]
  re_cover_ab = x$re_cover_ab
  field = unique(re_cover_ab$Field)
  plot = unique(re_cover_ab$Plot)
  fit_list = x$fit_data_prop
  
  
  df_l = lapply(fit_list, function(x){
    #x = fit_list[[1]]
    X_t_inter = x$X_t_inter
    X_top50 = as.data.frame(matrix(data = rep(0, 50*nrow(X_t_inter)),
                                   nrow = nrow(X_t_inter), ncol = 50))
    colnames(X_top50) = freq_top50_sps
    X_top50[,colnames(X_t_inter)] = X_t_inter
    X_t_intra = x$X_t_intra
    names(X_t_intra) = 'intra_sp'
...
fit_ranfield_top50_ages1_35_cover_prop = rbindlist(fit_ranfield_top50_ages1_35_cover_prop)
fit_ranfield_top50_ages1_35_cover_prop_l = split(fit_ranfield_top50_ages1_35_cover_prop, fit_ranfield_top50_ages1_35_cover_prop$focal)
fit_ranfield_top50_ages1_35_cover_prop_sp = lapply(fit_ranfield_top50_ages1_35_cover_prop_l,
                                        function(x){
                                          #x = fit_ranfield_top50_ages1_35_cover_prop_l[[1]]
                                          x_c = x %>% select(all_of(names(which(colSums(x[,9:ncol(x)]) != 0))))
                                          fit_list = list(
                                            N = nrow(x),
                                            P = 50,
                                            time_diff = x$time_diff,
                                            ...
                                            X_t_intra = as.vector(x$intra_sp),
                                            X_t_inter = as.matrix(x_c),
                                            Y_t1 = x$Y_t1
                                          )
                                        })
save(fit_ranfield_top50_ages1_35_cover_prop_sp,
     file = 'code/data preparation/transformed data/fit_ranfield_top50_ages1_35_cover_prop_sp.RData')


#### top50 prepare for random_field fitting_prop #####
library(tidyr)
load("code/data preparation/transformed data/fit_fp_top50_ages1_35_prop.RData")
freq_top50_sps = colnames(sp_cover_f1_ages1_35_top50)[10:ncol(sp_cover_f1_ages1_35_top50)]
fit_ranfield_top50_ages1_35_prop = lapply(fit_fp_top50_ages1_35_prop, function(x){
  #x = fit_fp_top50_ages1_35_prop[[1]]
  re_cover_ab = x$re_cover_ab
  field = unique(re_cover_ab$Field)
  plot = unique(re_cover_ab$Plot)
  fit_list = x$fit_data_prop
...
fit_ranfield_top50_ages1_35_prop = rbindlist(fit_ranfield_top50_ages1_35_prop)
fit_ranfield_top50_ages1_35_prop_l = split(fit_ranfield_top50_ages1_35_prop, fit_ranfield_top50_ages1_35_prop$focal)
fit_ranfield_top50_ages1_35_prop_sp = lapply(fit_ranfield_top50_ages1_35_prop_l,
                                        function(x){
                                          fit_list = list(
                                            N = nrow(x),
                                            P = 50,
...
save(fit_ranfield_top50_ages1_35_prop_sp,
     file = 'code/data preparation/transformed data/fit_ranfield_top50_ages1_35_prop_sp.RData')
