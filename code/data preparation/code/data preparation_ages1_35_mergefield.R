...
sp_cover_f1_ages1_35_field = sp_cover_f1 %>% filter(fake_age %in% c(1:35))

sp_cover_f1_ages1_35_field_fp = split(sp_cover_f1_ages1_35_field,
                                sp_cover_f1_ages1_35_field$f_p)
re_cover_ab_f1_ages1_35_fp = lapply(sp_cover_f1_ages1_35_field_fp, 
                                    function(w){
...
sp_cover_f1_ages1_35_field = as.data.frame(sp_cover_f1_ages1_35_field %>% group_by(#Absolute_year,             
                                             #Year,
                                             #fake_year,
                                             #Age,
                                             fake_age,
...
sp_cover_f1_ages1_35_field_l = split(sp_cover_f1_ages1_35_field,
                                     sp_cover_f1_ages1_35_field$Field)


######## Filter top species for fit under plot/field and all field level
### top 50: top25 exotics and top25 natives
freq_sps = apply(sp_cover_f1_ages1_35_field[,c(3:ncol(sp_cover_f1_ages1_35_field))],
...
sp_cover_f1_ages1_35_field_top50 = cbind(as.data.frame(sp_cover_f1_ages1_35_field[,c(1:2)]),
                                   as.data.frame(sp_cover_f1_ages1_35_field %>% select(names(freq_sps_top_50))))
sp_cover_f1_ages1_35_field_top50_l = split(sp_cover_f1_ages1_35_field_top50,
                                            sp_cover_f1_ages1_35_field_top50$Field)
...
sp_cover_ages1_35_top25_exotic = cbind(sp_cover_f1_ages1_35_field[,c(1:9)],
                                       sp_cover_f1_ages1_35_field %>% select(names(freq_sps_top_25exotic_allnative)))
...
freq_sps = apply(sp_cover_f1_ages1_35_field[,c(10:ncol(sp_cover_f1_ages1_35_field))], 2, function(x){length(x[x>0])})
freq_sps_exotic = freq_sps[(trait %>% filter(Origin == 'Exotic'))$Species]
freq_sps_exotic = freq_sps_exotic[which(!is.na(freq_sps_exotic))]
freq_sps_native = freq_sps[(trait %>% filter(Origin == 'Native'))$Species] 
freq_sps_native = freq_sps_native[which(!is.na(freq_sps_native))]
...
sp_cover_f1_ages1_35_field_top40 = cbind(sp_cover_f1_ages1_35_field[,c(1:9)],
                                   sp_cover_f1_ages1_35_field %>% select(names(freq_sps_top_40)))
sp_cover_f1_ages1_35_field_top40_fp = split(sp_cover_f1_ages1_35_field_top40, sp_cover_f1_ages1_35_field_top40$f_p)
...
sp_cover_ages1_35_top20_exotic = cbind(sp_cover_f1_ages1_35_field[,c(1:9)],
                                       sp_cover_f1_ages1_35_field %>% select(names(freq_sps_top_20exotic_allnative)))
...
sp_cover_f1_ages1_35_field_top30 = cbind(sp_cover_f1_ages1_35_field[,c(1:9)],
                                   sp_cover_f1_ages1_35_field %>% select(names(freq_sps_top_30)))
sp_cover_f1_ages1_35_field_top30_fp = split(sp_cover_f1_ages1_35_field_top30, sp_cover_f1_ages1_35_field_top30$f_p)
...
sp_cover_ages1_35_top15_exotic = cbind(sp_cover_f1_ages1_35_field[,c(1:9)],
                                       sp_cover_f1_ages1_35_field %>% select(names(freq_sps_top_15exotic_allnative)))
...
###### invasion stage at plot level
invasion_stages_ages1_35_fp = lapply(re_cover_ab_f1_ages1_35_fp, 
                                     fit_prepa_top,
                                     posi_sp_start = 10)
save(invasion_stages_ages1_35_fp,
     file = "code/data preparation/transformed data/invasion_stages_ages1_35_fp.RData")
...
fit_field_top50_ages1_35 = lapply(sp_cover_f1_ages1_35_field_top50_l,
                               fit_prepa_top_merge,
                               posi_sp_start = 3)

intro_sp_top50_early_suc = sapply(fit_field_top50_ages1_35,
...
domin_sp_top50_early_suc = sapply(fit_field_top50_ages1_35,
                                  function(x){length(strsplit(x[["stage_sp_name"]][4], ', ')[[1]])})
length(intro_sp_top50_early_suc[which(intro_sp_top50_early_suc == 0)])/length(intro_sp_top50_early_suc)
mean(sapply(fit_field_top50_ages1_35,
            function(x){ncol(x[["re_cover_ab"]])-9})) ## mean 37 species
abundance_pro = mean(sapply(fit_field_top50_ages1_35,
                            function(x){sum(x[["re_cover_ab"]][,c(10:ncol(x[["re_cover_ab"]]))])})/abundance_raw)

richness_pro = mean(sapply(fit_field_top50_ages1_35,
                           function(x){ncol(x[["re_cover_ab"]][,c(10:ncol(x[["re_cover_ab"]]))])})/richness_raw)

save(fit_field_top50_ages1_35,
     file = "code/data preparation/transformed data/fit_field_top50_ages1_35.RData")

#### top50 prepare for random_field fitting #####
library(tidyr)
load("code/data preparation/transformed data/fit_field_top50_ages1_35.RData")
freq_top50_sps = colnames(sp_cover_f1_ages1_35_field_top50)[10:ncol(sp_cover_f1_ages1_35_field_top50)]
fit_ranfield_top50_ages1_35 = lapply(fit_field_top50_ages1_35, function(x){
  #x = fit_field_top50_ages1_35[[1]]
  re_cover_ab = x[[2]]
  field = unique(re_cover_ab$Field)
  plot = unique(re_cover_ab$Plot)
  fit_list = x[[3]]
  X = fit_list[[1]]$X
  X_top50 = as.data.frame(matrix(data = rep(0, 50*nrow(X)),
                                 nrow = nrow(X), ncol = 50))
  colnames(X_top50) = freq_top50_sps
  X_top50[,colnames(X)] = X
  sps_ID = which(freq_top50_sps %in% colnames(X))
  
  df_l = lapply(fit_list, function(x){z = cbind(
    field = field,
    plot = plot,
    f_p = plot,
    focal = gsub('[0-9]+',
    ...
    X_top50)})
  df = rbindlist(df_l)
  fit_list_all = list(sps_ID, df)
  return(df)
})

fit_ranfield_top50_ages1_35 = rbindlist(fit_ranfield_top50_ages1_35)
fit_ranfield_top50_ages1_35_l = split(fit_ranfield_top50_ages1_35, fit_ranfield_top50_ages1_35$focal)
fit_ranfield_top50_ages1_35_sp = lapply(fit_ranfield_top50_ages1_35_l,
                                        function(x){
                                          fit_list = list(
                                            N = nrow(x),
                                            P = 50,
                                            time_diff = x$time_diff,
                                            ftime = length(unique(x$time)),
                                            ffield = length(unique(x$field)),
                                            X = as.matrix(x[,8:ncol(x)]),
                                            G = x$G,
                                            time = x$time,
                                            field = as.factor(x$field)
                                          )
                                        })
save(fit_ranfield_top50_ages1_35_sp,
     file = 'code/data preparation/transformed data/fit_ranfield_top50_ages1_35_sp.RData')

## top 40 sps for early succession (ages 1-35)
fit_fp_top40_ages1_35 = lapply(sp_cover_f1_ages1_35_field_top40_fp, fit_prepa_top)
...
fit_fp_top30_ages1_35 = lapply(sp_cover_f1_ages1_35_field_top30_fp, fit_prepa_top)
