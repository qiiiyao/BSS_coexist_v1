...
######## filter top 50 species 
sp_cover_only = sp_cover_f2[,10:ncol(sp_cover_f2)]
...
#plants3 = log(plants3)
pyear = split(sp_cover_focal_all, sp_cover_f2$fake_age)
pchange = list()
pchange.logit = list()
pchange.log = list()
...
pchange.all2 = data.frame("f_p" = rep(unique(sp_cover_f2$f_p), 24),
                          "Field" = rep(unlist(lapply(strsplit(unique(sp_cover_f2$f_p),
                                                               '_'),
                                                   function(x){
                                              x[1]
                           })), 24),
                          "Plot" = rep(unlist(lapply(strsplit(unique(sp_cover_f2$f_p),
                                                              '_'),
                                                  function(x){
                                                    x[2]
                                                  })), 24),
                          "Year_change" = rep(Year_change, each = 480),
                          "Yeart" = rep(1:24, each = 480), pchange.all) 
...
dd = brm(as.formula(paste(yy[1], " ~ 0 + Intercept + ", 
                          paste(top50.short,
                                collapse="+"), 
                          '+ (1|Plot/Field)')),
         data = pchange.all2, warmup = 1000, iter = 3000, 
         chains = 4, 
         #core = 2,
...
save.image('code/data preparation/fit_ricker_randomfield.rdata')
