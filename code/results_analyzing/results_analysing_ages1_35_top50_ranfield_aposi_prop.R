...
rm(list = ls())
load("D:/R projects/BSS/results/fit_results/BSS_exclude_trees_raw/ran_t_f_ages1_35_top50_normal_posi_prop/inter_all_c_alltime.rdata")
...
Fig.1_compare_estab_original = ggplot(NULL) +
  ...
  scale_shape_manual(values = c(16,1),
                     labels = c("intro.estab_native" = "Successful",
                                "intro.noestab_native" = "Failed"),
                     name = ' ')+
  coord_cartesian(xlim=c(-15, 2.5),ylim=c(-20, 5))+ # just control the lab, not the data
...
Fig.1_compare_estab_original_2 = ggplot(NULL) +
  geom_point(data = (inter_all_forestab  %>% 
                            distinct(sp_pair, .keep_all = T)),
                  mapping = aes(x = nd,y = fd,color = stage_ij_estab ,
                                shape = stage_ij_estab )
  )+
...
colnames(inter_all_fordomin_msd) = c('stage', 'nd_mean2',
                                     'fd_mean2', 'nd_sd2',
                                     'fd_sd2')
inter_all_fordomin$ni_se = inter_all_fordomin$ni_sd/sqrt(1000)
inter_all_fordomin$fi_se = inter_all_fordomin$fi_sd/sqrt(1000)
Fig.1_compare_domin_original = ggplot(NULL) +
  ...
  scale_shape_manual(values = c(16,1),
                     labels = c("estab.domin_native" = "Successful",
                                "estab.nodomin_native" = "Failed"),
                     name = ' ')+
  coord_cartesian(xlim=c(-10, 10),ylim=c(-20, 10))+ # just control the lab, not the data
  annotate(geom="text", x=-7, y=200, family = 'Arial',
           label='Dominance')+
  labs(x = "Niche difference", 
       y = "  ") +
