...
############### Fast Start ####################
load("D:/R projects/BSS/results/fit_results/inter_all_c_alltime.rdata")
library(dplyr)
library(betareg)
library(stringr)
library(data.table)
...
ggsave('results/figures_alltime/Fig.1.svg',
       plot=Fig.1, 
...
ggsave(plot = Fig.1_split_stage_original,
       'results/figures_alltime/Fig.1_split_stage_original.svg',
...
ggsave(plot = Fig.1_split_stage_original_forppt,
       'results/figures_alltime/Fig.1_split_stage_original_forppt.svg',
...
#ggsave('results/figures_alltime/Fig.1_compare_domin.svg',
...
ggsave(plot = Fig.1_split_stage_2,
       'results/figures_alltime/Fig.1_split_stage_2.svg',
       width = 165,height = 340, dpi = 300, units = 'cm',
       limitsize = F)
...
plot(inter_all_c$nd, inter_all_c$Phylo_dis)
...
save(vcv_tree_sparse, file = 'code/results_analyzing/analysing_alltime_data/vcv_tree_sparse.rdata')

### Above model run in linux due to RAM limited!
#### nd ~ pd + fd ####
...
#### Plot nd lgfd ~ pd+fd ####
load("D:/R projects/BSS/results/analysing_results/mod_nd_pd_fd.rdata")
load("D:/R projects/BSS/results/analysing_results/mod_lgfd_pd_fd.rdata")
mod_nd_pd_fd_fixed = mod_nd_pd_fd$summary.fixed
mod_lgfd_pd_fd_fixed = mod_lgfd_pd_fd$summary.fitted
coeffs_nd_pdgower = data.frame(stage = c('PD', 'FD'),
                        nd = c(mod_nd_pd_fd_fixed[2,1],
                               mod_nd_pd_fd_fixed[3,1]),
                        lt = c(mod_nd_pd_fd_fixed[2,5],
                               mod_nd_pd_fd_fixed[3,5]),
                        ut = c(mod_nd_pd_fd_fixed[2,3],
                               mod_nd_pd_fd_fixed[3,3]))

coeffs_lgfd_pdgower = data.frame(stage = c('PD', 'FD'),
                                 lgfd = c(mod_lgfd_pd_fd_fixed[2,1],
                                        mod_lgfd_pd_fd_fixed[3,1]),
                                 lt = c(mod_lgfd_pd_fd_fixed[2,5],
                                        mod_lgfd_pd_fd_fixed[3,5]),
                                 ut = c(mod_lgfd_pd_fd_fixed[2,3],
                                        mod_lgfd_pd_fd_fixed[3,3]))
library(ggplot2)
library(ggplot2)
...
p_lgfd_pdgower = ggplot(coeffs_lgfd_pdgower, aes(x=lgfd, y=stage)) + 
  geom_pointrange(aes(xmin=lt, xmax=ut), linewidth = 2,
                  fatten = 40) + 
  geom_vline(aes(xintercept=0), colour="#BB0000", linetype="dashed", size = 2)+
  ggtitle('log10(FD)')+
  ...
  theme(plot.title = element_text(hjust = 0.5,
        ...
        axis.title.x = element_text(margin=margin(t=20),
                                    color = 'black'))
library(ggpubr)
p_ndfd_pdgower = ggarrange(p_nd_pdgower, p_lgfd_pdgower,
                              nrow = 1, ncol = 2,
                              labels = c('a)', 'b)'),
                              font.label = list(size = 50))

ggsave(plot = p_ndfd_pdgower,
       'results/figures_alltime/p_ndfd_pdgower.svg',
       width = 200,height = 80,dpi = 300, units = 'cm',
       limitsize = F)


######## Analyse invasion success ########
inter_all_c$ra_m_fit_t_i = as.numeric(inter_all_c$ra_m_fit_t_i)
inter_all_c$ra_m_fit_t_j = as.numeric(inter_all_c$ra_m_fit_t_j)
...
str(dat_suc_sp)
save(dat_suc_sp,
     file = 'code/results_analyzing/analysing_alltime_data/dat_suc_sp.rdata')

############ Fast start for analyzing invasion success probability ~ nd+fd #########
load('code/results_analyzing/analysing_alltime_data/dat_suc_sp.rdata')
library(ordinal)
library(lme4)
library(INLA)
library(brinla)
...
car::vif(
  glmer(estab ~ mnd.a + mlgfd.a + (1|species) + (1|f_p),
        family=binomial,data=dat_suc_sp)
)
## VIF < 3
with(dat_suc_sp,cor(mnd.a,mlgfd.a))
...
#### Establishment
pc_prior = list(prec=list("pc.prec", param=c(0.1,0.01)))
mod_suc_sp_ab_2 = inla(estab ~ mnd.a + mlgfd.a +
                         f(field, model="iid", hyper = pc_prior) +
                         f(f_p, model="iid",  hyper = pc_prior),
                       control.compute = list(dic=T,waic=T,cpo=T),
                       quantiles=c(0.025,0.5,0.975),
                       family="binomial", data=dat_suc_sp)
mod_suc_sp_ab_3 = inla(estab ~ mnd.a + mlgfd.a +
                         ...
                         quantiles=c(0.025,0.5,0.975),
                         family="binomial", data=dat_suc_sp)
c(mod_suc_sp_ab_2$waic$waic, mod_suc_sp_ab_3$waic$waic)
summary(mod_suc_sp_ab_2)
summary(mod_suc_sp_ab_3)
bri.hyperpar.summary(mod_suc_sp_ab_2)
bri.hyperpar.summary(mod_suc_sp_ab_3)
mod_suc_sp_ab_2$summary.fixed
mod_suc_sp_ab_3_fixed = mod_suc_sp_ab_3$summary.fixed
...
mod_dom_sp_ab_2 = inla(domin ~ mnd.a + mlgfd.a +
                         f(field, model="iid", hyper = pc_prior) +
                         f(f_p, model="iid",  hyper = pc_prior),
                       control.compute = list(dic=T,waic=T,cpo=T),
                       quantiles=c(0.025, 0.5, 0.975),
                       family="binomial", data=dat_dom_sp)
mod_dom_sp_ab_3 = inla(domin ~ mnd.a + mlgfd.a +
                         f(species, model = "iid", hyper = pc_prior)+
                         f(field, model="iid", hyper = pc_prior) +
                         f(f_p, model="iid", hyper = pc_prior),
                       control.compute = list(dic=T,waic=T,cpo=T),
                       quantiles=c(0.025, 0.5, 0.975),
                       family="binomial", data=dat_dom_sp)
c(mod_dom_sp_ab_2$waic$waic, mod_dom_sp_ab_3$waic$waic)
summary(mod_dom_sp_ab_2)
summary(mod_dom_sp_ab_3)
bri.hyperpar.summary(mod_dom_sp_ab_2)
bri.hyperpar.summary(mod_dom_sp_ab_3)
mod_dom_sp_ab_2$summary.fixed
mod_dom_sp_ab_3_fixed = mod_dom_sp_ab_3$summary.fixed
...
mod_sp_all_ab_1_brms = brm(stage_level ~ mnd.a + mlgfd.a+ (1|species)+
                (1|ffield/fplot), data=dat_suc_sp,
              family=cumulative("logit", link_disc = "log",
                                threshold = "flexible"),
              chains = 4,warmup = 1000, iter = 3000, refresh = 100,   
              control = list(max_treedepth = 20,
                             adapt_delta = 0.99), cores = 10)
load('code/results_analyzing/analysing_alltime_data/mod_sp_all_ab_1_brms.rdata')
mod_sp_all_ab_1_brms_fixed = fixef(mod_sp_all_ab_1_brms,
                                   probs = c(0.025, 0.975),
                                   summary = T,
                                   robust = T)
#save(mod_sp_all_ab_1_brms,
#     file = 'code/results_analyzing/analysing_alltime_data/mod_sp_all_ab_1_brms.rdata')
anova(mod_sp_all_ab, mod_sp_all_ab_1)

####### Analyse for mnd mfd ###########
### Establishment
pc_prior = list(prec=list("pc.prec", param=c(0.1,0.01)))
mod_suc_sp_2 = inla(estab ~ mnd + mlgfd +
                         f(field, model="iid", hyper = pc_prior) +
                         f(f_p, model="iid",  hyper = pc_prior),
                       control.compute = list(dic=T,waic=T,cpo=T),
                       quantiles=c(0.025, 0.5, 0.975),
                       family="binomial", data=dat_suc_sp)
mod_suc_sp_3 = inla(estab ~ mnd + mlgfd +
                         f(species, model = "iid", hyper = pc_prior)+
                         f(field, model="iid", hyper = pc_prior) +
                         f(f_p, model="iid", hyper = pc_prior),
                       control.compute = list(dic=T,waic=T,cpo=T),
                       quantiles=c(0.025, 0.5, 0.975),
                       family="binomial", data=dat_suc_sp)
c(mod_suc_sp_2$waic$waic, mod_suc_sp_3$waic$waic)
summary(mod_suc_sp_2)
summary(mod_suc_sp_3)
bri.hyperpar.summary(mod_suc_sp_2)
bri.hyperpar.summary(mod_suc_sp_3)
mod_suc_sp_2$summary.fixed
mod_suc_sp_3_fixed = mod_suc_sp_3$summary.fixed

#### Domin
dat_dom_sp = dat_suc_sp %>% filter(stage %in% c('establish', 'dominant'))
mod_dom_sp_2 = inla(domin ~ mnd + mlgfd +
                         f(field, model="iid", hyper = pc_prior) +
                         f(f_p, model="iid",  hyper = pc_prior),
                       control.compute = list(dic=T,waic=T,cpo=T),
                       quantiles=c(0.025, 0.5, 0.975),
                       family="binomial", data=dat_dom_sp)
mod_dom_sp_3 = inla(domin ~ mnd + mlgfd +
                         f(species, model = "iid", hyper = pc_prior)+
                         f(field, model="iid", hyper = pc_prior) +
                         f(f_p, model="iid", hyper = pc_prior),
                       control.compute = list(dic=T,waic=T,cpo=T),
                       quantiles=c(0.025, 0.5, 0.975),
                       family="binomial", data=dat_dom_sp)
c(mod_dom_sp_2$waic$waic, mod_dom_sp_3$waic$waic)
summary(mod_dom_sp_2)
summary(mod_dom_sp_3)
bri.hyperpar.summary(mod_dom_sp_2)
bri.hyperpar.summary(mod_dom_sp_3)
mod_dom_sp_2$summary.fixed
mod_dom_sp_3_fixed = mod_dom_sp_3$summary.fixed

#### All stages
# Normal method
dat_suc_sp$stage_level = ordered(dat_suc_sp$stage_level)
mod_sp_all = clm(stage_level ~ mnd + mlgfd,
                    data = dat_suc_sp)
mod_sp_all_1 = clmm(stage_level ~ mnd + mlgfd+ (1|species)+
                         (1|ffield/fplot),
                       data = dat_suc_sp)
summary(mod_sp_all)
summary(mod_sp_all_1)

# Bayseian method
options(mc.cores = parallel::detectCores(logical = F)) 
mod_sp_all_1_brms = brm(stage_level ~ mnd + mlgfd+ (1|species)+
                             (1|ffield/fplot), data=dat_suc_sp,
                           family=cumulative("logit", link_disc = "log",
                                             threshold = "flexible"),
                           chains = 4,warmup = 1000, iter = 3000, refresh = 100,   
                           control = list(max_treedepth = 20,
                                          adapt_delta = 0.99), cores = 10)
#save(mod_sp_all_1_brms,
#     file = 'code/results_analyzing/analysing_alltime_data/mod_sp_all_1_brms.rdata')

load('code/results_analyzing/analysing_alltime_data/mod_sp_all_1_brms.rdata')
mod_sp_all_1_brms_fixed = fixef(mod_sp_all_1_brms,
                                   probs = c(0.025, 0.975),
                                   summary = T,
                                   robust = T)

anova(mod_sp_all, mod_sp_all_1)

####### Analyse for mnnd mnlgfd ###########
### Establishment
pc_prior = list(prec=list("pc.prec", param=c(0.1,0.01)))
mod_suc_sp_mntd_2 = inla(estab ~ mnnd + mnlgfd +
                      f(field, model="iid", hyper = pc_prior) +
                      f(f_p, model="iid",  hyper = pc_prior),
                    control.compute = list(dic=T,waic=T,cpo=T),
                    quantiles=c(0.025, 0.5, 0.975),
                    family="binomial", data=dat_suc_sp)
mod_suc_sp_mntd_3 = inla(estab ~ mnnd + mnlgfd +
                      f(species, model = "iid", hyper = pc_prior)+
                      f(field, model="iid", hyper = pc_prior) +
                      f(f_p, model="iid", hyper = pc_prior),
                    control.compute = list(dic=T,waic=T,cpo=T),
                    quantiles=c(0.025, 0.5, 0.975),
                    family="binomial", data=dat_suc_sp)
c(mod_suc_sp_mntd_2$waic$waic, mod_suc_sp_mntd_3$waic$waic)
summary(mod_suc_sp_mntd_2)
summary(mod_suc_sp_mntd_3)
bri.hyperpar.summary(mod_suc_sp_mntd_2)
bri.hyperpar.summary(mod_suc_sp_mntd_3)
mod_suc_sp_mntd_2$summary.fixed
mod_suc_sp_mntd_3_fixed = mod_suc_sp_mntd_3$summary.fixed

#### Domin
dat_dom_sp = dat_suc_sp %>% filter(stage %in% c('establish', 'dominant'))
mod_dom_sp_mntd_2 = inla(domin ~ mnnd + mnlgfd +
                      f(field, model="iid", hyper = pc_prior) +
                      f(f_p, model="iid",  hyper = pc_prior),
                    control.compute = list(dic=T,waic=T,cpo=T),
                    quantiles=c(0.025, 0.5, 0.975),
                    family="binomial", data=dat_dom_sp)
mod_dom_sp_mntd_3 = inla(domin ~ mnnd + mnlgfd +
                      f(species, model = "iid", hyper = pc_prior)+
                      f(field, model="iid", hyper = pc_prior) +
                      f(f_p, model="iid", hyper = pc_prior),
                    control.compute = list(dic=T,waic=T,cpo=T),
                    quantiles=c(0.025, 0.5, 0.975),
                    family="binomial", data=dat_dom_sp)
c(mod_dom_sp_mntd_2$waic$waic, mod_dom_sp_mntd_3$waic$waic)
summary(mod_dom_sp_mntd_2)
summary(mod_dom_sp_mntd_3)
bri.hyperpar.summary(mod_dom_sp_mntd_2)
bri.hyperpar.summary(mod_dom_sp_mntd_3)
mod_dom_sp_mntd_2$summary.fixed
mod_dom_sp_mntd_3_fixed = mod_dom_sp_mntd_3$summary.fixed

#### All stages
# Normal method
mod_sp_all_mntd = clm(stage_level ~ mnnd + mnlgfd,
                 data = dat_suc_sp)
mod_sp_all_mntd_1 = clmm(stage_level ~ mnnd + mnlgfd+ (1|species)+
                      (1|ffield/fplot),
                    data = dat_suc_sp)
summary(mod_sp_all_mntd)
summary(mod_sp_all_mntd_1)

# Bayseian method
options(mc.cores = parallel::detectCores(logical = F)) 
mod_sp_all_mntd_1_brms = brm(stage_level ~ mnnd + mnlgfd+ (1|species)+
                          (1|ffield/fplot), data=dat_suc_sp,
                        family=cumulative("logit", link_disc = "log",
                                          threshold = "flexible"),
                        chains = 4,warmup = 1000, iter = 3000, refresh = 100,   
                        control = list(max_treedepth = 20,
                                       adapt_delta = 0.99), cores = 10)
#anova(mod_sp_all_mntd, mod_sp_all_mntd_1)
#save(mod_sp_all_mntd_1_brms,
#     file = 'code/results_analyzing/analysing_alltime_data/mod_sp_all_mntd_1_brms.rdata')
load('code/results_analyzing/analysing_alltime_data/mod_sp_all_mntd_1_brms.rdata')
mod_sp_all_mntd_1_brms_fixed = fixef(mod_sp_all_mntd_1_brms,
                                     probs = c(0.025, 0.975),
                                     summary = T,
                                     robust = T)
...
coeffs_mnd = data.frame(stage = c('establishment', 'dominant', 'overall'),
                        nd = c(mod_suc_sp_3_fixed[2,1],
                               mod_dom_sp_3_fixed[2,1],
                               mod_sp_all_1_brms_fixed[3,1]),
                        ut = c(mod_suc_sp_3_fixed[2,5],
                               mod_dom_sp_3_fixed[2,5],
                               mod_sp_all_1_brms_fixed[3,4]),
                        lt = c(mod_suc_sp_3_fixed[2,3],
                               mod_dom_sp_3_fixed[2,3],
                               mod_sp_all_1_brms_fixed[3,3]))
coeffs_mlgfd = data.frame(stage = c('establishment', 'dominant', 'overall'),
                          lgfd = c(mod_suc_sp_3_fixed[3,1],
                                 mod_dom_sp_3_fixed[3,1],
                                 mod_sp_all_1_brms_fixed[4,1]),
                          ut = c(mod_suc_sp_3_fixed[3,5],
                                 mod_dom_sp_3_fixed[3,5],
                                 mod_sp_all_1_brms_fixed[4,4]),
                          lt = c(mod_suc_sp_3_fixed[3,3],
                                 mod_dom_sp_3_fixed[3,3],
                                 mod_sp_all_1_brms_fixed[4,3]))
library(ggplot2)
...
ggsave(plot = Fig.2_ab,
       'results/figures_alltime/Fig.2_ab.svg',
       width = 200,height = 80, dpi = 300, units = 'cm',
       limitsize = F)

ggsave(plot = Fig.2_mnd,
       'results/figures_alltime/Fig.2_mnd.svg',
       width = 200,height = 80, dpi = 300, units = 'cm',
       limitsize = F)

ggsave(plot = Fig.2_mnnd,
       'results/figures_alltime/Fig.2_mnnd.svg',
       width = 200,height = 80, dpi = 300, units = 'cm',
       limitsize = F)
...
#inla.model_all1_md.a = inla(estab~mnd.a+mlgfd.a+mpd.a+mconti_func_d.a+
#                         f(species, model = "iid", hyper = pc_prior)+
#                         f(field, model="iid", hyper = pc_prior) +
#                         f(f_p, model="iid", hyper = pc_prior)+
#                         f(species_1, model="generic0",
#                           Cmatrix=vcv_tree_sparse,
#                           values = sp_names, hyper=pc_prior),
#                       family="binomial",data=dat_suc_sps,
#                       control.compute=list(dic=T,waic=T,cpo=T),
#                       quantiles=c(0.025,0.15,0.5,0.85,0.975))
#summary(inla.model_all1_md.a)

#inla.model_all1_mnd = inla(estab~mnnd+mnlgfd+mntd+mnconti_func_d+
#                              f(species, model = "iid", hyper = pc_prior)+
 #                             f(field, model="iid", hyper = pc_prior) +
 #                             f(f_p, model="iid", hyper = pc_prior)+
 #                             f(species_1, model="generic0",
  ...
  #                          control.compute=list(dic=T,waic=T,cpo=T),
  #                          quantiles=c(0.025,0.15,0.5,0.85,0.975))
#summary(inla.model_all1_mnd)
#c(inla.model_all1$waic$waic, inla.model_all1_md.a$waic$waic,
#  inla.model_all1_mnd$waic$waic) ### Using the inla.model_all1
...
#inla.model_all1 = inla(domin~mnd+mlgfd+mpd+mconti_func_d+
#                         f(species, model = "iid", hyper = pc_prior)+
#                         f(field, model="iid", hyper = pc_prior) +
#                         f(f_p, model="iid", hyper = pc_prior)+
#                         f(species_1, model="generic0",
#                           Cmatrix=vcv_tree_sparse,
 ...
 #                      family="binomial",data=dat_dom_sp,
  #                     control.compute=list(dic=T,waic=T,cpo=T),
  #                     quantiles=c(0.025,0.15,0.5,0.85,0.975))
#summary(inla.model_all1)

#inla.model_all1_md.a = inla(domin~mnd.a+mlgfd.a+mpd.a+mconti_func_d.a+
 #                             f(species, model = "iid", hyper = pc_prior)+
 #                             f(field, model="iid", hyper = pc_prior) +
 #                             f(f_p, model="iid", hyper = pc_prior)+
   #                           f(species_1, model="generic0",
...
#c(inla.model_all1$waic$waic, inla.model_all1_md.a$waic$waic,
#  inla.model_all1_mnd$waic$waic) ### Using the inla.model_all1_mnd

# plot 
domin_data.inla.all.varied_intercept1 = domin_inla.model_all1_mnd$summary.fixed%>%
  rownames_to_column()%>%.[-1,]%>%rename(lower1="0.025quant",lower2="0.15quant",median="0.5quant",upper2="0.85quant",upper1="0.975quant")

domin_data.inla.all.varied_intercept = domin_data.inla.all.varied_intercept1%>%
  mutate(rowname=c("mnnd","mnlgfd","mntd","mnconti_func_d"))%>%
  mutate(percent=abs(mean)/sum(abs(mean)))
...
domin_inla.varied.intercept.plot =
  ...
  geom_linerange(aes(xmin=lower2,xmax=upper2),size=12)+
  geom_vline(xintercept=0,linetype=2,color="grey40",linewidth=6)+
  scale_y_discrete(limits=arrange(domin_data.inla.all.varied_intercept,percent)$rowname)+#按照%R2的排序排。图是从下到上对应这些值
  scale_x_continuous(name="Standardized effects",limits=c(-1,1))+
  scale_color_viridis_d()+
  guides(color="none")+
  theme(panel.background = element_rect(fill = 'white',color = 'black',linewidth = 2),
...
ggsave(plot = Estab_dominant_all,
       'results/figures_alltime/Estab_dominant_all.svg',
...
ggsave(plot = Estab_dominant_all_forppt,
       'results/figures_alltime/Estab_dominant_all_forppt.svg',
...
ggsave(plot = Fig.3,
       'results/figures_alltime/Fig.3.svg',
       width = 160,height = 240,dpi = 300, units = 'cm',
       limitsize = F)

ggsave(plot = Fig.3.ab,
       'results/figures_alltime/Fig.3.ab.svg',
       width = 160,height = 240,dpi = 300, units = 'cm',
       limitsize = F)
...
intro_imp_inla.varied.intercept.plot =
  ...
  geom_linerange(aes(xmin=lower2,xmax=upper2),size=12)+
  geom_vline(xintercept=0,linetype=2,color="grey40",linewidth=6)+
  scale_y_discrete(limits=arrange(intro_imp_data.inla.all.varied_intercept,percent)$rowname)+#按照%R2的排序排。图是从下到上对应这些值
  scale_x_continuous(name="Standardized effects",limits=c(-1,1))+
  scale_color_viridis_d()+
  guides(color="none")+
  theme(panel.background = element_rect(fill = 'white',color = 'black',linewidth = 0.5),
...
# R2%
intro_imp_inla.varied.intercept.R2.plot = 
  ggplot(data=intro_imp_data.inla.all.varied_intercept,aes(percent,rowname,fill=rowname))+
  geom_bar(stat="identity",width=0.8)+
  geom_text(aes(label=paste(sprintf("%.1f",percent*100),"%",sep="")),
            hjust=-0.1,size=80)+
  theme_void()+
  scale_y_discrete(limits=arrange(intro_imp_data.inla.all.varied_intercept,percent)$rowname)+
  scale_fill_viridis_d()+
  theme(plot.margin=unit(c(0.4,0,1.8,-0.3),units="lines"))+
  xlim(0,0.5)+
  guides(fill="none")
...
# Merge effect size + R2 
intro_implished_all = ggarrange(intro_imp_inla.varied.intercept.plot,
                            intro_imp_inla.varied.intercept.R2.plot,
                            widths=c(2,1.2))
...
dat_imp_sp_estab = dat_imp_sp_sum_c %>%
  filter(estab == 1 & domin == 0 & !is.na(gain_loss_2))
mod_ra_change_sp_estab_ab = inla(mean.relative.change_2 ~ mnd.a + mlgfd.a 
                                 + f(species, model="iid", hyper = gammaprior)
                                 + f(field, model="iid", hyper = gammaprior)
                                 + f(field:plot, model="iid", hyper = gammaprior),
                                 control.compute = list(dic=T,waic=T,cpo=T),
                                 family="gaussian",
                                 data = dat_imp_sp_estab)
summary(mod_ra_change_sp_estab_ab)
mod_ra_change_sp_estab_ab_fixed = mod_ra_change_sp_estab_ab$summary.fixed

mod_ra_change_sp_estab =  inla(mean.relative.change_2 ~ mnd + mlgfd
                               + f(species, model="iid", hyper = gammaprior)
                               + f(field, model="iid", hyper = gammaprior)
                               + f(field:plot, model="iid", hyper = gammaprior),
                               control.compute = list(dic=T,waic=T,cpo=T),
                               family="gaussian",
                               data = dat_imp_sp_estab)
summary(mod_ra_change_sp_estab)
mod_ra_change_sp_estab_fixed = mod_ra_change_sp_estab$summary.fixed

## Dominant ##
dat_imp_sp_domin = dat_imp_sp_sum_c %>% filter(domin == 1) 
mod_ra_change_sp_domin_ab = inla(mean.relative.change_2 ~ mnd.a + mlgfd.a 
                                 + f(species, model="iid", hyper = gammaprior)
                                 + f(field, model="iid", hyper = gammaprior)
                                 + f(field:plot, model="iid", hyper = gammaprior),
                                 control.compute = list(dic=T,waic=T,cpo=T),
                                 family="gaussian",
                                 data = dat_imp_sp_domin)
summary(mod_ra_change_sp_domin_ab)
mod_ra_change_sp_domin_ab_fixed = mod_ra_change_sp_domin_ab$summary.fixed

mod_ra_change_sp_domin = inla(mean.relative.change_2 ~ mnd + mlgfd
                               + f(species, model="iid", hyper = gammaprior)
                               + f(field, model="iid", hyper = gammaprior)
                               + f(field:plot, model="iid", hyper = gammaprior),
                               control.compute = list(dic=T,waic=T,cpo=T),
                               family="gaussian",
                               data = dat_imp_sp_domin)
summary(mod_ra_change_sp_domin)
mod_ra_change_sp_domin_fixed = mod_ra_change_sp_domin$summary.fixed

## ALL three stages ##
dat_imp_sp_all = dat_imp_sp_sum_c %>% filter(stage != 'introduce' &
                                               stage != 'establish' &
                                               stage != 'dominant') 
mod_ra_change_sp_all_ab = inla(mean.relative.change_2 ~ mnd.a + mlgfd.a 
                                 + f(species, model="iid", hyper = gammaprior)
                                 + f(field, model="iid", hyper = gammaprior)
                                 + f(field:plot, model="iid", hyper = gammaprior),
                                 control.compute = list(dic=T,waic=T,cpo=T),
                                 family="gaussian",
                                 data = dat_imp_sp_all)
summary(mod_ra_change_sp_all_ab)
mod_ra_change_sp_all_ab_fixed = mod_ra_change_sp_all_ab$summary.fixed

mod_ra_change_sp_all = inla(mean.relative.change_2 ~ mnd + mlgfd
                              + f(species, model="iid", hyper = gammaprior)
                              + f(field, model="iid", hyper = gammaprior)
                              + f(field:plot, model="iid", hyper = gammaprior),
                              control.compute = list(dic=T,waic=T,cpo=T),
                              family="gaussian",
                              data = dat_imp_sp_all)
summary(mod_ra_change_sp_all)
mod_ra_change_sp_all_fixed = mod_ra_change_sp_all$summary.fixed

# plot invasion impact
coeffs_mnd_ra_change = data.frame(stage = c('introduce', 
                                  'establishment',
                                  'dominant',
                                  'overall'),
                        nd = c(mod_ra_change_sp_intro_fixed[2,1],
                               mod_ra_change_sp_estab_fixed[2,1],
                               mod_ra_change_sp_domin_fixed[2,1],
                               mod_ra_change_sp_all_fixed[2,1]),
                        ut = c(mod_ra_change_sp_intro_fixed[2,5],
                               mod_ra_change_sp_estab_fixed[2,5],
                               mod_ra_change_sp_domin_fixed[2,5],
                               mod_ra_change_sp_all_fixed[2,5]),
                        lt = c(mod_ra_change_sp_intro_fixed[2,3],
                               mod_ra_change_sp_estab_fixed[2,3],
                               mod_ra_change_sp_domin_fixed[2,3],
                               mod_ra_change_sp_all_fixed[2,3]))

coeffs_mlgfd_ra_change = data.frame(stage = c('introduce', 
                                              'establishment',
                                              'dominant',
                                              'overall'),
                                    lgfd = c(mod_ra_change_sp_intro_fixed[3,1],
                                           mod_ra_change_sp_estab_fixed[3,1],
                                           mod_ra_change_sp_domin_fixed[3,1],
                                           mod_ra_change_sp_all_fixed[3,1]),
                                    ut = c(mod_ra_change_sp_intro_fixed[3,5],
                                           mod_ra_change_sp_estab_fixed[3,5],
                                           mod_ra_change_sp_domin_fixed[3,5],
                                           mod_ra_change_sp_all_fixed[3,5]),
                                    lt = c(mod_ra_change_sp_intro_fixed[3,3],
                                           mod_ra_change_sp_estab_fixed[3,3],
                                           mod_ra_change_sp_domin_fixed[3,3],
                                           mod_ra_change_sp_all_fixed[3,3]))

library(ggplot2)
p_mod_ra_change_nd = ggplot(coeffs_mnd_ra_change, aes(x=nd, y=stage)) + 
  geom_pointrange(aes(xmin=lt, xmax=ut), linewidth = 2,
                  fatten = 40) + 
  geom_vline(aes(xintercept=0), colour="#BB0000", linetype="dashed", size = 2) +
  ggtitle('MND')+
...
p_mod_ra_change_lgfd = ggplot(coeffs_mlgfd_ra_change, aes(x=lgfd, y=stage)) + 
  geom_pointrange(aes(xmin=lt, xmax=ut), linewidth = 2,
                  fatten = 40) + 
  geom_vline(aes(xintercept=0), colour="#BB0000", linetype="dashed", size = 2)+
  ggtitle('MlgFD')+
...
# plot invasion success ab 
coeffs_mnd_ab_ra_change = data.frame(stage = c('introduce', 
                                            'establishment',
                                            'dominant',
                                            'overall'),
                                  nd = c(mod_ra_change_sp_intro_ab_fixed[2,1],
                                         mod_ra_change_sp_estab_ab_fixed[2,1],
                                         mod_ra_change_sp_domin_ab_fixed[2,1],
                                         mod_ra_change_sp_all_ab_fixed[2,1]),
                                  ut = c(mod_ra_change_sp_intro_ab_fixed[2,5],
                                         mod_ra_change_sp_estab_ab_fixed[2,5],
                                         mod_ra_change_sp_domin_ab_fixed[2,5],
                                         mod_ra_change_sp_all_ab_fixed[2,5]),
                                  lt = c(mod_ra_change_sp_intro_ab_fixed[2,3],
                                         mod_ra_change_sp_estab_ab_fixed[2,3],
                                         mod_ra_change_sp_domin_ab_fixed[2,3],
                                         mod_ra_change_sp_all_ab_fixed[2,3]))

coeffs_mlgfd_ab_ra_change = data.frame(stage = c('introduce', 
                                              'establishment',
                                              'dominant',
                                              'overall'),
                                    lgfd = c(mod_ra_change_sp_intro_ab_fixed[3,1],
                                           mod_ra_change_sp_estab_ab_fixed[3,1],
                                           mod_ra_change_sp_domin_ab_fixed[3,1],
                                           mod_ra_change_sp_all_ab_fixed[3,1]),
                                    ut = c(mod_ra_change_sp_intro_ab_fixed[3,5],
                                           mod_ra_change_sp_estab_ab_fixed[3,5],
                                           mod_ra_change_sp_domin_ab_fixed[3,5],
                                           mod_ra_change_sp_all_ab_fixed[3,5]),
                                    lt = c(mod_ra_change_sp_intro_ab_fixed[3,3],
                                           mod_ra_change_sp_estab_ab_fixed[3,3],
                                           mod_ra_change_sp_domin_ab_fixed[3,3],
                                           mod_ra_change_sp_all_ab_fixed[3,3]))
library(ggplot2)
p_mod_ra_change_nd_ab = ggplot(coeffs_mnd_ab_ra_change, aes(x=nd, y=stage)) + 
  geom_pointrange(aes(xmin=lt, xmax=ut), linewidth = 2,
                  fatten = 40) + 
  geom_vline(aes(xintercept=0), colour="#BB0000", linetype="dashed", size = 2) +
  ggtitle(paste0('M',
...
p_mod_ra_change_lgfd_ab = ggplot(coeffs_mlgfd_ab_ra_change,
                                 aes(x=lgfd, y=stage)) + 
  geom_pointrange(aes(xmin=lt, xmax=ut), linewidth = 2,
                  fatten = 40) + 
  geom_vline(aes(xintercept=0), colour="#BB0000", linetype="dashed", size = 2)+
...
Fig.3_ra = ggarrange(p_mod_ra_change_nd, p_mod_ra_change_lgfd, nrow = 1, ncol = 2,
                  labels = c('a)', 'b)'), font.label = list(size = 200))
Fig.3_ra.ab = ggarrange(p_mod_ra_change_nd_ab, p_mod_ra_change_lgfd_ab, nrow = 1, ncol = 2,
                     labels = c('a)', 'b)'), font.label = list(size = 200))

ggsave(plot = Fig.3_ra,
       'results/figures_alltime/Fig.3_ra.svg',
       width = 280,height = 100,dpi = 300, units = 'cm',
       limitsize = F)

ggsave(plot = Fig.3_ra.ab,
       'results/figures_alltime/Fig.3_ra.ab.svg',
       width = 280,height = 100,dpi = 300, units = 'cm',
       limitsize = F)
...
source('code/results_analyzing/function/Li_suc_prepare.R')
source('code/results_analyzing/function/mpd_calculate.R')

#### Plot level 
#dat_suc_sp_all = lapply(sp_cover_f1_fp, Li_suc_prepare)
#save(dat_suc_sp_all, file = 'code/results_analyzing/pd_invasion_dat.rdata')

load('code/results_analyzing/pd_invasion_dat.rdata')
glmm_intro_all = lapply(pd_invasion_dat, function(x){
  intro = x$intro
  return(intro)
})
glmm_intro_all = rbindlist(glmm_intro_all)
glmm_intro_all$plot = as.integer(glmm_intro_all$plot)
glmm_intro_all$field = as.integer(glmm_intro_all$field)
glmm_intro_all$year = as.integer(glmm_intro_all$year)
glmm_intro_all = arrange(glmm_intro_all, intro, field, plot, year)

glmm_intro_all$plot = sapply(stringr::str_split(glmm_intro_all$f_p, '_'),
                             function(x){x[2]})
glmm_intro_all$f_p = as.factor(glmm_intro_all$f_p)
glmm_int_my = glmer(intro~mpd+ (1 | field/f_p)+(1 | year) ,
                   family=binomial(link = "logit"),
                   data=glmm_intro_all)
summ1 = summary(glmm_int_my)

glmm_int_my.a = glmer(intro~mpd.a+ (1 | field/f_p)+(1 | year) ,
                    family=binomial(link = "logit"),
                    data=glmm_intro_all)
summary(glmm_int_my.a)

glmm_int_mn.a = glmer(intro~mntd+ (1 | field/f_p)+(1 | year) ,
                      family=binomial(link = "logit"),
                      data=glmm_intro_all)
summary(glmm_int_mn.a)

glmm_estab_all = lapply(pd_invasion_dat, function(x){
  estab = x$estab
  return(estab)
})
glmm_estab_all = rbindlist(glmm_estab_all)
glmm_estab_all$field = as.integer(glmm_estab_all$field)
glmm_estab_all$year = as.integer(glmm_estab_all$year)
glmm_estab_all = arrange(glmm_estab_all, estab, field, plot, year)

glmm_estab_all$plot = sapply(stringr::str_split(glmm_estab_all$f_p, '_'),
                             function(x){x[2]})
glmm_estab_all$f_p = as.factor(glmm_estab_all$f_p)
glmm_est_my = glmer(estab~mpd+ (1 | field/f_p)+(1 | year) ,
                    family=binomial(link = "logit"),
                    data=glmm_estab_all,
                    nAGQ = 9)
summ1 = summary(glmm_est_my)

glmm_est_my.a = glmer(estab~mpd.a+ (1 | field/f_p)+(1 | year) ,
                      family=binomial(link = "logit"),
                      data=glmm_estab_all)
summary(glmm_est_my.a)

glmm_est_mn.a = glmer(estab~mntd+ (1 | field/f_p)+(1 | year) ,
                      family=binomial(link = "logit"),
                      data=glmm_estab_all)
summary(glmm_est_mn.a)

####### using fitted species to analyse relationship between intro ~ PD

glmm_int_my.a = glmer(estab~mpd.a+ (1 | field/f_p)+(1 | year) ,
                      family=binomial(link = "logit"),
                      data=inter_all_c_alltime)
summary(glmm_int_my.a)
