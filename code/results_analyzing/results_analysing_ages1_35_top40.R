...
save(inter_all_c,
     file = 'results/fit_results/BSS_exclude_trees_raw/plot_ages1_35_top40/inter_all_c_alltime.rdata')
...
lincombs.data.domin.mnd.a_single = data.frame(mnd.a=seq(min(dat_suc_sp$mnd.a),
                                                        max(dat_suc_sp$mnd.a),
                                                        length=100))
...
(domin.mnd.a_single.logistic=ggplot(data=lincombs.data.domin.mnd.a_single,aes(x=mnd.a, y=predicted.value))+
    ...
    scale_color_gradient(low = turbo(4)[2],
                         high = turbo(4)[1])+
    #theme(plot.margin=unit(c(0.4,0,0.4,0.4),units="lines"))+
    geom_point(data=dat_suc_sp, aes(x=mnd.a, y=domin),
               color = stata_pal("s2color")(4)[3],
               shape=1,
               alpha = 0.2,
               position=position_jitter(height=0.02))+
    ...
    annotate(geom="text",x=c(0,0),y=c(0.90,0.80),
             label=c("italic(β)['ND'[ab]] == '-2.34'",
                     "'95%CI' == '[-2.79, -1.88]'"),
             parse=T,size=3.5)+
    theme_regular()
)
#mnd.a       -2.3353    -2.7904    -1.8802



### get dominance predict data for mlgfd.a
lincombs.data.domin.mlgfd.a_single = data.frame(mlgfd.a=seq(min(dat_suc_sp$mlgfd.a),
                                                            max(dat_suc_sp$mlgfd.a),
                                                            length=100))
...
inla.model_lincombs.domin.mlgfd.a_single$inla.model$summary.fixed[c(1,3,5)]%>%round(4)##Extracting effects and confidence intervals for prediction curves from raw data
...
(domin.mlgfd.a_single.logistic=ggplot(data=lincombs.data.domin.mlgfd.a_single,aes(x=mlgfd.a, y=predicted.value))+
    ...
    scale_color_gradient(low = turbo(4)[2],
                         high = turbo(4)[1])+
    #theme(plot.margin=unit(c(0.4,0,0.4,0.4),units="lines"))+
    geom_point(data=dat_suc_sp, aes(x=mlgfd.a, y=domin),
               color = stata_pal("s2color")(4)[2],
               shape=1,
               alpha = 0.2,
               position=position_jitter(height=0.02))+
    ...
    annotate(geom="text",x=c(0,0),y=c(0.90,0.80),
             label=c("italic(β)['RFD'[ab]] == '8.11'",
                     "'95%CI' == '[7.29, 8.92]'"),
             parse=T,size=3.5)+
    theme_regular()
)
#mlgfd.a      8.1060     7.2895     8.9226
...
(domin.mpd.a_single.logistic=ggplot(data=lincombs.data.domin.mpd.a_single,aes(x=mpd.a_all, y=predicted.value))+
    ...
    annotate(geom="text",x=c(200,200),y=c(0.90,0.80),
             label=c("italic(β)['MPD'[ab]] == '-0.0033'", "'95%CI' == '[-0.0064, -0.0002]'"),
             parse=T,size=3.5)+
    theme_regular()
)
#-0.0033    -0.0064    -0.0002
...
(domin.mfunc_d.a_single.logistic=ggplot(data=lincombs.data.domin.mfunc_d.a_single,
    ...
    annotate(geom="text",x=c(0.25,0.25),y=c(0.90,0.80),
             label=c("italic(β)['MFD'[ab]] == '5.9488'",
                     "'95%CI' == '[4.2006, 7.6840]'"),
             parse=T,size=3.5)+
    theme_regular()
)
...
emf('results/figures_ages1_35_top50/all_species_d/Fig.2.md.a.emf',
...
emf('results/figures_ages1_35_top50/all_species_d/Fig.2.md.a_single.emf',
    width = 20, height = 20, coordDPI = 1200, 
    emfPlusFontToPath=TRUE, # setting emfPlusFontToPath=TRUE to 
    # ensure text looks correct on the viewing system
    units = 'cm')
...
emf('results/figures_ages1_35_top50/all_species_d/Fig.2.md.emf',
    width = 20, height = 20, coordDPI = 1200, 
    emfPlusFontToPath=TRUE, # setting emfPlusFontToPath=TRUE to 
    # ensure text looks correct on the viewing system
    units = 'cm')
...
emf('results/figures_ages1_35_top50/sem_estab_md.a_all_plot_all.emf',
...
emf('results/figures_ages1_35_top50/sem_domin_md.a_all_plot_all.emf',
    width=16, height=12, coordDPI = 1200, 
    emfPlusFontToPath=TRUE, # setting emfPlusFontToPath=TRUE to 
    # ensure text looks correct on the viewing system
    units = 'cm')
