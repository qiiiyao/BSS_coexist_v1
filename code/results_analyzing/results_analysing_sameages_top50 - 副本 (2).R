...
############### Fast Start ####################
load("D:/R projects/BSS/results/fit_results/BSS_exculde_trees_raw/plot_sameages_top50/inter_all_c_alltime.rdata")
library(dplyr)
library(betareg)
library(stringr)
library(data.table)
...
Fig.1_compare_estab_original = ggplot(NULL) +
  ...
  theme_test() + 
  theme(legend.position = c(0.24, 0.88),legend.key = element_blank(),
...
ggsave(plot = Fig.1_split_stage_original,
       'results/figures_sameages_top50/Fig.1_split_stage_original.svg',
       width = 16,height = 34, units = 'cm',
       limitsize = F)
...
ggsave(plot = Fig.1_split_stage_original_forppt,
       'results/figures_sameages_top50/Fig.1_split_stage_original_forppt.svg',
...
save(vcv_tree_n_a,
     file = 'code/results_analyzing/analysing_sameages_top50_data/vcv_tree_n_a.rdata')
...
save(vcv_tree_sparse, file = 'code/results_analyzing/analysing_sameages_top50_data/vcv_tree_sparse.rdata')
...
ggsave(plot = plo_ndfd_pd_func_d,
       'results/figures_sameages_top50/plo_ndfd_pd_func_d.jpg',
...
ggsave(plot = p_ndfd_pdgower,
       'results/figures_sameages_top50/p_ndfd_pdgower.svg',
       width = 200,height = 80,dpi = 300, units = 'cm',
       limitsize = F)
ggsave(plot = p_ndfd_pdgower,
       'results/figures_sameages_top50/p_ndfd_pdgower_forppt.svg',
...
str(dat_suc_sp)
save(dat_suc_sp,
     file = 'code/results_analyzing/analysing_sameages_top50_data/dat_suc_sp.rdata')
...
##### 由于运存不足，所以模型在linux上运行，因此直接跳转至画图部分
...
car::vif(
  glmer(estab ~ mnd.a + mlgfd.a + (1|species) + (1|f_p) + (1|field),
...
mod_suc_sp_ab_md.a = pglmm(estab ~ mnd.a+mlgfd.a+(1|species) + 
...
mod_suc_sp_ab_md = pglmm(estab ~ mnd+mlgfd+(1|species)
                         + (1|f_p) + (1|field),
                         data = dat_suc_sps,
                         family = "binomial", cov_ranef = list(species = tree),
                         bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
...
summary(mod_suc_sp_ab_md$inla.model)

mod_suc_sp_ab_mnd = pglmm(estab ~ mnnd+mnlgfd+(1|species)
                          + (1|f_p) + (1|field)
                          ,
                          data = dat_suc_sps,
                          family = "binomial", cov_ranef = list(species = tree),
...
c(mod_suc_sp_ab_md$WAIC, mod_suc_sp_ab_md.a$WAIC,
  mod_suc_sp_ab_mnd$WAIC) ## 选择mean differences,因为其waic最低

#-------------------预测曲线,用原始数据。画出inla.model_all1中introduction_native.mpd与Introduction_outcome关系的偏逻辑斯蒂回归曲线，即INLA中所谓的预测。采用lincombs
lincombs.data.estab.nd = data.frame(mnd=seq(min(dat_suc_sp$mnd),max(dat_suc_sp$mnd),length=100),
                                    mlgfd=mean(dat_suc_sp$mlgfd))

lincombs.matrix.estab.nd=model.matrix(~mnd+mlgfd,
                                      data=lincombs.data.estab.nd)
lincombs.matrix.estab.nd=as.data.frame(lincombs.matrix.estab.nd)
lincombs.estab.nd=inla.make.lincombs(lincombs.matrix.estab.nd)

inla.model_lincombs.estab.nd = pglmm(estab ~ mnd+mlgfd+(1|species) + 
                                       (1|f_p) + (1|field), data = dat_suc_sp,
                                     family = "binomial", cov_ranef = list(species = tree),
                                     bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
                                                                                 config = TRUE),
                                                          quantiles=c(0.025,0.5,0.975),
                                                          lincomb=lincombs.estab.nd,
                                                          control.predictor=list(compute=T)),
                                     bayes = T)

lincombs.posterior.estab.nd = inla.model_lincombs.estab.nd$inla.model$marginals.lincomb.derived[c(1:100)]
inla.model_lincombs.estab.nd$inla.model$summary.fixed[c(1,3,5)]%>%round(2)##提取原始数据预测曲线的效应和置信区间

#inla.model1_lincombs.estab.nd$summary.lincomb,这个是logit尺度的，不能直接用，需要用后验分布转为原始尺度，再计算均值和置信。下面方法转：
lincombs.data.estab.nd$predicted.value=unlist(lapply(lincombs.posterior.estab.nd,function(x)inla.emarginal(fun=plogis,x)))
lincombs.data.estab.nd$lower=unlist(lapply(lincombs.posterior.estab.nd,function(x)inla.qmarginal(0.025,inla.tmarginal(fun=plogis,x))))
lincombs.data.estab.nd$upper=unlist(lapply(lincombs.posterior.estab.nd,function(x)inla.qmarginal(0.975,inla.tmarginal(fun=plogis,x))))
#lincombs.data.estab.nd

#save(lincombs.data.estab.nd, 
#     file = 'code/results_analyzing/analysing_sameages_top50_data/lincombs.data.estab.nd.rdata')

#备注：plogis()将后验转为了原始尺度,inla.emarginal(fun,x)计算转后的期望值即均值，打包为一个函数，
#inla.tmarginal(fun,x)将后验转，而后inla.qmarginal()将转后的后验计算置信区间。上面inla.emarginal()是一下完成了这两步操作。
#将整个函数应用于列表的每一成分，之后再通过unlist将结果转为向量，加入lincombs.data,开始作图。

#现在画出偏逻辑斯蒂回归曲线
load("code/results_analyzing/analysing_sameages_top50_data/lincombs.data.estab.nd.rdata")
(estab.nd.partial.logistic=ggplot(data=lincombs.data.estab.nd,
    ...
    annotate(geom="text",x=c(0.33,0.33),y=c(0.80,0.70),
             label=c("italic(β)['I-N MND'] == -2.40",
                     "'95%CI' == '[-3.23, -1.57]'"),parse=T,size=3.5)
)


#-------------------预测曲线,用原始数据。画出inla.model_all1中introduction_native.mfd与Introduction_outcome关系的偏逻辑斯蒂回归曲线，即INLA中所谓的预测。采用lincombs
...
lincombs.matrix.estab.lgfd=model.matrix(~mnd+mlgfd,
...
inla.model_lincombs.estab.lgfd = pglmm(estab ~ mnd+mlgfd+(1|species) + 
                                         (1|f_p) + (1|field), data = dat_suc_sp,
                                       family = "binomial", cov_ranef = list(species = tree),
                                       bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
                                                                                   config = TRUE),
...
inla.model_lincombs.estab.lgfd$inla.model$summary.fixed[c(1,3,5)]%>%round(2)##提取原始数据预测曲线的效应和置信区间

#inla.model1_lincombs.estab.lgfd$summary.lincomb,这个是logit尺度的，不能直接用，需要用后验分布转为原始尺度，再计算均值和置信。下面方法转：
...
save(lincombs.data.estab.lgfd, 
     file = 'code/results_analyzing/analysing_sameages_top50_data/lincombs.data.estab.lgfd.rdata')
...
#现在画出偏逻辑斯蒂回归曲线
load("code/results_analyzing/analysing_sameages_top50_data/lincombs.data.estab.lgfd.rdata")
(estab.lgfd.partial.logistic=ggplot(data=lincombs.data.estab.lgfd,aes(x=mlgfd, y=predicted.value))+
    ...
    annotate(geom="text",x=c(0,0),y=c(0.80,0.70),
             label=c("italic(β)['I-N MFit_D'] == 2.16", "'95%CI' == '[1.33, 2.98]'"),parse=T,size=3.5)
)

#### 拟合系统发育与功能性状距离模型，并与ND/lgFD做比较
mod_suc_sp_ab_mpd_mfunc_d = pglmm(estab ~ mpd+mconti_func_d+(1|species)
                                  + (1|f_p) + (1|field),
                                  data = dat_suc_sps,
                                  family = "binomial", cov_ranef = list(species = tree),
                                  bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
                                                       ...
                                                       quantiles=c(0.025,0.5,0.975)),
                                  bayes = T)
mod_suc_sp_ab_mpd.a_mfunc_d.a = pglmm(estab ~ mpd.a+mconti_func_d.a+(1|species)
                                      + (1|f_p) + (1|field),
                                      data = dat_suc_sps,
                                      family = "binomial", cov_ranef = list(species = tree),
                                      bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
                                                           ...
                                                           quantiles=c(0.025,0.5,0.975)),
                                      bayes = T)
mod_suc_sp_ab_mntd_mfunc_d = pglmm(estab ~ mntd+mnconti_func_d+(1|species)
                                   + (1|f_p) + (1|field),
                                   data = dat_suc_sps,
                                   family = "binomial", cov_ranef = list(species = tree),
                                   bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
...
c(mod_suc_sp_ab_mpd_mfunc_d$WAIC, mod_suc_sp_ab_mpd.a_mfunc_d.a$WAIC,
  mod_suc_sp_ab_mntd_mfunc_d$WAIC)

c(mod_suc_sp_ab_mpd.a_mfunc_d.a$WAIC, mod_suc_sp_ab_md$WAIC)
## [1] 5879.075 5689.448
...
mod_dom_sp_ab_md.a = pglmm(domin ~ mnd.a+mlgfd.a+(1|species) + 
...
mod_dom_sp_ab_md = pglmm(domin ~ mnd+mlgfd+(1|species)
                         + (1|f_p) + (1|field),
                         data = dat_dom_sps,
                         family = "binomial", cov_ranef = list(species = tree),
                         bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
...
mod_dom_sp_ab_mnd = pglmm(domin ~ mnnd+mnlgfd+(1|species)
                          + (1|f_p) + (1|field),
                          data = dat_dom_sps,
                          family = "binomial", cov_ranef = list(species = tree),
                          bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
...
c(mod_dom_sp_ab_md$WAIC, mod_dom_sp_ab_md.a$WAIC,
  mod_dom_sp_ab_mnd$WAIC) ## 选择mean nearest differences,因为其waic最低
summary(mod_dom_sp_ab_mnd)

#-------------------预测曲线,用原始数据。画出inla.model_all1中introduction_native.mnnd与dominance_outcome关系的偏逻辑斯蒂回归曲线，即INLA中所谓的预测。采用lincombs
lincombs.data.domin.nd = data.frame(mnnd=seq(min(dat_dom_sp$mnnd),max(dat_dom_sp$mnnd),length=100),
                                    mnlgfd=mean(dat_dom_sp$mnlgfd))

lincombs.matrix.domin.nd=model.matrix(~mnnd+mnlgfd,
...
inla.model_lincombs.domin.nd = pglmm(domin ~ mnnd+mnlgfd+(1|species) + 
...
inla.model_lincombs.domin.nd$inla.model$summary.fixed[c(1,3,5)]%>%round(2)##提取原始数据预测曲线的效应和置信区间

#inla.model1_lincombs.domin.nd$summary.lincomb,这个是logit尺度的，不能直接用，需要用后验分布转为原始尺度，再计算均值和置信。下面方法转：
lincombs.data.domin.nd$predicted.value=unlist(lapply(lincombs.posterior.domin.nd,function(x)inla.emarginal(fun=plogis,x)))
...
#     file = 'code/results_analyzing/analysing_sameages_top50_data/lincombs.data.domin.nd.rdata')
...
#现在画出偏逻辑斯蒂回归曲线
load("code/results_analyzing/analysing_sameages_top50_data/lincombs.data.domin.nd.rdata")
(domin.nd.partial.logistic=ggplot(data=lincombs.data.domin.nd,
    ...
    geom_point(data=dat_dom_sp, aes(x=mnnd, y=domin,
                                    color = domin),shape=1,
              position=position_jitter(height=0.02))+
    labs(x="Established-native niche difference",y='Dominance probability')+
    annotate(geom="text",x=c(-0.25,-0.25),y=c(0.80,0.70),
             label=c("italic(β)['I-N MNND'] == -0.81",
                     "'95%CI' == '[-1.34, -0.28]'"),parse=T,size=3.5)
)


#-------------------预测曲线,用原始数据。画出inla.model_all1中introduction_native.mfd与Introduction_outcome关系的偏逻辑斯蒂回归曲线，即INLA中所谓的预测。采用lincombs
lincombs.data.domin.lgfd = data.frame(mnlgfd=seq(min(dat_dom_sp$mnlgfd),max(dat_dom_sp$mnlgfd),length=100),
                                      mnnd=mean(dat_dom_sp$mnnd))

lincombs.matrix.domin.lgfd=model.matrix(~mnnd+mnlgfd,
...
inla.model_lincombs.domin.lgfd = pglmm(domin ~ mnnd+mnlgfd+(1|species) + 
                                         (1|f_p) + (1|field), data = dat_dom_sp,
                                       family = "binomial", cov_ranef = list(species = tree),
                                       bayes_options = list(control.compute = list(dic=T,waic=T,cpo=T,
                                                                                   config = TRUE),
...
inla.model_lincombs.domin.lgfd$inla.model$summary.fixed[c(1,3,5)]%>%round(2)##提取原始数据预测曲线的效应和置信区间

#inla.model1_lincombs.domin.lgfd$summary.lincomb,这个是logit尺度的，不能直接用，需要用后验分布转为原始尺度，再计算均值和置信。下面方法转：
...
save(lincombs.data.domin.lgfd, 
     file = 'code/results_analyzing/analysing_sameages_top50_data/lincombs.data.domin.lgfd.rdata')
...
#现在画出偏逻辑斯蒂回归曲线
load("code/results_analyzing/analysing_sameages_top50_data/lincombs.data.domin.lgfd.rdata")
(domin.lgfd.partial.logistic=ggplot(data=lincombs.data.domin.lgfd,aes(x=mnlgfd, y=predicted.value))+
    geom_ribbon(aes(ymin=lower,ymax=upper),
                fill='grey1',alpha=0.2)+
    geom_line(color='grey1',size=1)+
    theme_test()+
    ...
    annotate(geom="text",x=c(-0.25,-0.25),y=c(0.80,0.70),
             label=c("italic(β)['I-N MNFit_D'] == 3.09",
                     "'95%CI' == '[2.15, 4.03]'"),parse=T,size=3.5)
)


###### 图1：展示ND/FD如何影响物种成功定殖和取得优势，包括原始数据和偏回归结果的展示 #########
library(ggpubr)
...
ggsave(Fig.1, file = 'results/figures_sameages_top50/Fig.1_1.png',
       width = 25.2, height = 16.1, units = 'cm')
...
save(mod_sp_all_md_brms,
     file = 'code/results_analyzing/analysing_sameages_top50_data/mod_sp_all_md_brms.rdata')
...
save(mod_sp_all_md.a_brms,
     file = 'code/results_analyzing/analysing_sameages_top50_data/mod_sp_all_md.a_brms.rdata')
...
save(mod_sp_all_mnd_brms,
     file = 'code/results_analyzing/analysing_sameages_top50_data/mod_sp_all_mnd_brms.rdata')
...
ggsave(plot = Fig.2_ab,
       'results/figures_sameages_top50/Fig.2_ab.svg',
       width = 200,height = 80, dpi = 300, units = 'cm',
       limitsize = F)
ggsave(plot = Fig.2_mnd,
       'results/figures_sameages_top50/Fig.2_mnd.svg',
       width = 240,height = 80, dpi = 300, units = 'cm',
       limitsize = F)
ggsave(plot = Fig.2_mntd,
       'results/figures_sameages_top50/Fig.2_mnnd.svg',
       width = 200,height = 80, dpi = 300, units = 'cm',
       limitsize = F)
...
c(estab_model_all_md_conti_func_d$WAIC, estab_model_all_md.a_conti_func_d$WAIC,
  estab_model_all_mnd_conti_func_d$WAIC) ### 采用平均距离的模型
...
c(estab_model_all_md_func_d$WAIC, estab_model_all_md.a_func_d$WAIC,
  estab_model_all_mnd_func_d$WAIC) ### 采用平均距离的模型


c(estab_model_all_md_func_d$WAIC, estab_model_all_md_conti_func_d$WAIC) ## choose the conti_func_d model
...
estab_data.inla.all.md_conti_func_d_intercept = estab_data.inla.all.md_conti_func_d_intercept1%>%
  mutate(rowname=c("mnd","mfitness_d","mpd","mconti_func_d"))%>%
...
###### Plot for estab and domin ~ nd fitness_d pd func_d #######
# point + effect size
(estab_inla.varied.intercept.plot =
    ...
    geom_vline(xintercept=0,linetype=2,color="grey40",linewidth=1)+
    scale_y_discrete(limits=c("mnd", "mlgfd", "mpd", "mconti_func_d"))+
    scale_x_continuous(name="Standardized effects",limits=c(-2.2,1))+
...
ggsave(plot = Estab_dominant_all,
       'results/figures_sameages_top50/Estab_dominant_all.svg',
...
ggsave(plot = Estab_dominant_all_forppt,
       'results/figures_sameages_top50/Estab_dominant_all_forppt.svg',
...
save(dat_suc_sp_timeall,
     file = 'code/results_analyzing/analysing_sameages_top50_data/dat_suc_sp_timeall.rdata')
...
ggsave('results/figures_sameages_top50/success.mnd.mlgfd.effects_model.svg',
...
ggsave(plot = Fig.3,
       'results/figures_sameages_top50/Fig.3.svg',
       width = 160,height = 240,dpi = 300, units = 'cm',
       limitsize = F)

ggsave(plot = Fig.3.ab,
       'results/figures_sameages_top50/Fig.3.ab.svg',
       width = 160,height = 240,dpi = 300, units = 'cm',
       limitsize = F)
