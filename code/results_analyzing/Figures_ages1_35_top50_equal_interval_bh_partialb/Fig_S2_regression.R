...
load("results/fit_results/BSS_exclude_trees_raw/plot_ages1_35_top50_equal_interval_model_comparison/bh_allb/inter_all_c_alltime_rhat_105_newfd.rdata")
...
(estab.mnd.a_single.logistic=ggplot(data=lincombs.data.estab.mnd.a_single,aes(x=mnd.a, y=predicted.value))+
    ...
    annotate(geom="text",
             x=min(dat_suc_sp$mnd.a)+(max(dat_suc_sp$mnd.a)-min(dat_suc_sp$mnd.a))*0.74,
             y=c(0.25,0.15),
...
(domin.mnd.a_single.logistic=ggplot(data=lincombs.data.domin.mnd.a_single,aes(x=mnd.a, y=predicted.value))+
    ...
    annotate(geom="text",
             x=min(dat_dom_sp$mnd.a)+(max(dat_dom_sp$mnd.a)-min(dat_dom_sp$mnd.a))*0.74,
             y=c(0.25,0.15),
...
#### Fig.S1：EBH model results for demonstration of how Weighted Mean ND/RFD affects successful colonization and dominance of species, including presentation of raw data and single & multi-regression results ####
library(ggpubr)
library(export)
library(devEMF)
library(cowplot)
...
Fig.S2.all
emf('results/figures_ages1_35_top50_equal_interval_bh_partialb/Fig.S2.emf',
